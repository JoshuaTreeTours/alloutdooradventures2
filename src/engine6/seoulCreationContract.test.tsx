...
import { SEOUL_VIATOR_PUBLIC_PRODUCT_CODES } from "./seoulViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const seoulFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  SEOUL_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Seoul Engine6 creation contract", () => {
  it("covers every selected Seoul product code", () => {
    expect(seoulFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...SEOUL_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(seoulFixtures)("validates hardened contract for %s", fixture => {
