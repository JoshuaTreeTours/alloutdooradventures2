...
import {
  ...
  ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./acadiaNationalParkViatorPublicRatings";
import { ENGINE6_ACADIA_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const acadiaListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "maine" &&
    tour.destination.citySlug === "acadia-national-park" &&
    ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const acadiaResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/maine/acadia-national-park/") &&
    ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /\b(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion(?:\s+National\s+Park)?|Yosemite|Glacier National Park|\bGlacier\b|Bryce Canyon(?:\s+National\s+Park)?|Canyonlands(?:\s+National\s+Park)?|Arches(?:\s+National\s+Park)?|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Kauai|Waimea Canyon|Na Pali|Great Smoky|Grand Canyon|Hawaii Volcanoes|Kona|Hualalai|Kohala|Aspen|Hunter Creek|Roaring Fork|Wheeler Opera House|Boulder|Denver|Flatirons|Chautauqua|Pearl Street|Front Range|Austin|Lady Bird Lake|Texas State Capitol|Houston)\b/i;

describe("Acadia National Park Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Acadia cohort", () => {
    const engine6AcadiaTours = getToursByCityUnified(
      "maine",
      "acadia-national-park"
    ).filter(
      entry =>
        entry.tour.engine === "engine6" &&
        ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(
          entry.tour.productCode
        )
    );
    expect(engine6AcadiaTours).toHaveLength(
      ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-Acadia merchant feed rows", () => {
  ...
  it("keeps Acadia listing and governed descriptions free of unrelated destination bleed", () => {
    ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified(
      ...
      expect(governedDescription, productCode).toMatch(
        /\bAcadia(?: National Park)?\b/i
      );
      expect(cardDescription, productCode).toMatch(
        /\bAcadia(?: National Park)?\b/i
      );
  ...
  it("keeps Acadia public itinerary titles clean on rendered detail pages", () => {
...
describe("Acadia National Park Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_ACADIA_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Acadia National Park Engine6 itinerary title governance", () => {
  it("audits all Acadia listing products", () => {
    expect(acadiaListingTours).toHaveLength(
      ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(acadiaResolvedTours).toHaveLength(
      ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(acadiaResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = acadiaResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
