...
} from "./caboSanLucasViatorPublicRatings";
import { ENGINE6_CABO_SAN_LUCAS_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const caboSanLucasListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "mexico" &&
    tour.destination.citySlug === "cabo-san-lucas" &&
    CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const caboSanLucasResolvedTours = engine6ResolvedTours.filter(
  tour =>
...
describe("Cabo San Lucas Engine6 rating/review parity", () => {
  ...
  it.each(CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/mexico/cabo-san-lucas/");
      ...
      expect(listingEntry?.href).toContain("/mexico/cabo-san-lucas/");
  ...
  it("lists exactly the selected Engine6 cards for the Cabo San Lucas cohort", () => {
    const caboSanLucasListing = getToursByCityUnified("mexico", "cabo-san-lucas");
    expect(caboSanLucasListing).toHaveLength(CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      caboSanLucasListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(caboSanLucasListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Cabo San Lucas merchant feed rows", () => {
  ...
  it("keeps Cabo San Lucas listing and governed descriptions free of unrelated destination bleed", () => {
    CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bCabo San Lucas\b/i);
        expect(cardDescription, productCode).toMatch(/\bCabo San Lucas\b/i);
  ...
  it("keeps Cabo San Lucas public itinerary titles clean on rendered detail pages", () => {
...
describe("Cabo San Lucas Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_CABO_SAN_LUCAS_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Cabo San Lucas Engine6 itinerary title governance", () => {
  it("audits all Cabo San Lucas listing products", () => {
    expect(caboSanLucasListingTours).toHaveLength(
      CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(caboSanLucasResolvedTours).toHaveLength(
      CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(caboSanLucasResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(CABO_SAN_LUCAS_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = caboSanLucasResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
