...
import {
  ...
  HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./houstonViatorPublicRatings";
...
const houstonListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "texas" &&
    tour.destination.citySlug === "houston" &&
    HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const houstonResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/texas/houston/") &&
    HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /\b(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion|Yosemite|Glacier National Park|\bGlacier\b|Moab|Bryce|Arches|Canyonlands|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Kauai|Waimea Canyon|Na Pali|Great Smoky|Grand Canyon|Hawaii Volcanoes|Kona|Hualalai|Kohala|Aspen|Hunter Creek|Roaring Fork|Wheeler Opera House|Boulder|Denver|Flatirons|Chautauqua|Pearl Street|Front Range|Austin|Lady Bird Lake|Mt\.?\s*Bonnell|South Congress|Texas State Capitol)\b/i;

describe("Houston Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = HOUSTON_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("texas", "houston").find(
  ...
  it("lists exactly the selected Engine6 cards for the Houston cohort on the Houston city index", () => {
    const engine6HoustonTours = getToursByCityUnified("texas", "houston").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    expect(engine6HoustonTours).toHaveLength(
      HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-Houston merchant feed rows", () => {
  ...
  it("keeps Houston listing and governed descriptions free of unrelated destination bleed", () => {
    HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified("texas", "houston").find(
      ...
      expect(governedDescription, productCode).toMatch(/\bHouston\b/i);
      expect(cardDescription, productCode).toMatch(/\bHouston\b/i);
  ...
  it("keeps Houston public itinerary titles clean on rendered detail pages", () => {
...
describe("Houston Engine6 itinerary title governance", () => {
  it("audits all Houston listing products", () => {
    expect(houstonListingTours).toHaveLength(
      HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(houstonResolvedTours).toHaveLength(
      HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(houstonResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = houstonResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
