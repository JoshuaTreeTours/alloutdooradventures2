...
import { CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES } from "./cairnsViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const cairnsFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Cairns Engine6 creation contract", () => {
  it("covers every selected Cairns product code", () => {
    expect(cairnsFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(cairnsFixtures)("validates hardened contract for %s", fixture => {
