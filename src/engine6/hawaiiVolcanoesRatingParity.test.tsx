...
import {
  ...
  HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./hawaiiVolcanoesViatorPublicRatings";
...
const hawaiiVolcanoesListingTours = engine6ListingTours.filter(
  tour =>
    ...
    HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const hawaiiVolcanoesResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/hawaii/hawaii-volcanoes-national-park/") &&
    HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /\b(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion|Yosemite|Glacier|Moab|Bryce|Arches|Canyonlands|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Great Smoky|Grand Canyon)\b/i;

describe("Hawaii Volcanoes National Park Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = HAWAII_VOLCANOES_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly fourteen Engine6 cards for the Hawaii Volcanoes cohort on the city index", () => {
    const engine6HvnpTours = getToursByCityUnified(
      "hawaii",
      "hawaii-volcanoes-national-park"
    ).filter(
      entry =>
        entry.tour.engine === "engine6" &&
        HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES.includes(
          entry.tour.productCode
        )
    );
    expect(engine6HvnpTours).toHaveLength(14);
  });

  it("does not change non-HVNP merchant feed rows", () => {
  ...
  it("keeps HVNP listing and governed descriptions free of unrelated destination bleed", () => {
    HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified(
  ...
  it("keeps HVNP public itinerary titles clean on rendered detail pages", () => {
...
describe("Hawaii Volcanoes National Park Engine6 itinerary title governance", () => {
  it("audits all 14 HVNP listing products", () => {
    expect(hawaiiVolcanoesListingTours).toHaveLength(14);
    expect(hawaiiVolcanoesResolvedTours).toHaveLength(14);
    expect(
      hawaiiVolcanoesResolvedTours.map(tour => tour.productCode).sort()
    ).toEqual([...HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES].sort());
  });

  it.each(HAWAII_VOLCANOES_VIATOR_PUBLIC_PRODUCT_CODES)(
    "uses verified POI titles for resolved tour %s",
    productCode => {
      const tour = hawaiiVolcanoesResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
