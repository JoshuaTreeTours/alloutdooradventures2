...
import {
  ...
  BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./bryceCanyonNationalParkViatorPublicRatings";
import { ENGINE6_BRYCE_CANYON_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const bryceListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "utah" &&
    tour.destination.citySlug === "bryce-canyon-national-park" &&
    BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const bryceResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/utah/bryce-canyon-national-park/") &&
    BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
const DESTINATION_BLEED_PATTERN =
  /\b(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion(?:\s+National\s+Park)?|Yosemite|Glacier National Park|\bGlacier\b|Moab|Arches National Park|Canyonlands National Park|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Kauai|Waimea Canyon|Na Pali|Great Smoky|Grand Canyon|Hawaii Volcanoes|Kona|Hualalai|Kohala|Aspen|Hunter Creek|Roaring Fork|Wheeler Opera House|Boulder|Denver|Flatirons|Chautauqua|Pearl Street|Front Range|Austin|Lady Bird Lake|Texas State Capitol)\b/i;

describe("Bryce Canyon National Park Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Bryce Canyon cohort", () => {
    const engine6BryceTours = getToursByCityUnified(
      "utah",
      "bryce-canyon-national-park"
    ).filter(
      entry =>
    ...
    expect(engine6BryceTours).toHaveLength(
      BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-Bryce Canyon merchant feed rows", () => {
  ...
  it("keeps Bryce Canyon listing and governed descriptions free of unrelated destination bleed", () => {
    BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bBryce Canyon\b/i);
        expect(cardDescription, productCode).toMatch(/\bBryce Canyon\b/i);
  ...
  it("keeps Bryce Canyon public itinerary titles clean on rendered detail pages", () => {
...
describe("Bryce Canyon National Park Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_BRYCE_CANYON_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Bryce Canyon National Park Engine6 itinerary title governance", () => {
  it("audits all Bryce Canyon listing products", () => {
    expect(bryceListingTours).toHaveLength(
      BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(bryceResolvedTours).toHaveLength(
      BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(bryceResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = bryceResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
