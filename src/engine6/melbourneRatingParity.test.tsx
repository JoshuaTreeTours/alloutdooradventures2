...
import { ENGINE6_MELBOURNE_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const melbourneListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "australia" &&
    tour.destination.citySlug === "melbourne" &&
    MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const melbourneResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/australia/melbourne/") &&
    MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|\bLondon\b|\bParis\b|\bRome\b|\bVenice\b|Washington, D\.C\.|Washington D\.C\.|\bEdinburgh\b|\bBarcelona\b|\bAmsterdam\b|\bDublin\b|Arthur's Seat|Royal Mile|\bHighlands\b|\bCancun\b|Mexico City|Chichen Itza|Isla Mujeres|Puerto Vallarta|Marietas|Yelapa|Banderas Bay|Cabo San Lucas|El Arco|\bCusco\b|\bCuzco\b|Sacred Valley|Machu Picchu|Humantay|Rainbow Mountain|Vinicunca|Ollantaytambo|\bLima\b|Miraflores|Barranco|Rio de Janeiro|Christ the Redeemer|Sugar Loaf|Copacabana|Ipanema|Petrópolis|Petropolis|\bTokyo\b|Meiji Jingu|Asakusa|Shinjuku|Mount Fuji|Hakone|Kamakura|Nikko|\bOsaka\b|Dotonbori|Kuromon|Shinsekai|\bKyoto\b|Kinkaku-ji|Kiyomizu-dera|Fushimi Inari|Arashiyama|\bGion\b|\bBangkok\b|Ayutthaya|Grand Palace|Wat Pho|Wat Arun|Damnoen Saduak|Thonburi|\bThailand\b|\bSingapore\b|Marina Bay|Chinatown MRT|Pulau Ubin|Kampong Glam|\bBali\b|Ubud|Nusa Penida|Tanah Lot|Tirta Empul|\bSeoul\b|Gyeongbokgung|Namsan|Han River|Nami Island|Bukhansan|Gwangjang|\bSydney\b|Circular Quay|Bondi Beach|Blue Mountains|Sydney Opera House|Harbour Bridge|The Rocks|\bCairns\b|Great Barrier Reef|Daintree|Kuranda|Green Island|Fitzroy Island|Atherton Tablelands|Cape Tribulation|Reef Fleet Terminal|\bQueenstown\b|\bGlenorchy\b|Fiordland|Milford Sound|Doubtful Sound|\bNew Zealand\b|Melbourne Beach/i;

describe("Melbourne Engine6 rating/review parity", () => {
  ...
  it.each(MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = MELBOURNE_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      expect(tour?.canonicalPath).toContain("/australia/melbourne/");
      expect(tour?.state).toBe("Australia");
      expect(listingEntry?.tour.destination.state).toBe("Australia");
      expect(listingEntry?.tour.destination.stateSlug).toBe("australia");
      expect(listingEntry?.tour.destination.country).toBe("Australia");
      expect(listingEntry?.tour.destination.countryCode).toBe("AU");
      expect(listingEntry?.href).toContain("/australia/melbourne/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("AU");
      expect(placeNode?.address?.addressRegion).toBe("Australia");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("Australia");
      ...
      const expectedUsd = MELBOURNE_VIATOR_PUBLIC_USD_FROM_PRICES[productCode];
  ...
  it("covers USD From$ for every selected Melbourne product", () => {
    expect(Object.keys(MELBOURNE_VIATOR_PUBLIC_USD_FROM_PRICES).sort()).toEqual(
      [...MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("lists exactly the selected Engine6 cards for the Melbourne cohort", () => {
    const melbourneListing = getToursByCityUnified(
      "australia",
      "melbourne"
    );
    expect(melbourneListing).toHaveLength(
      MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      melbourneListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(
      melbourneListing.map(entry => entry.tour.productCode).sort()
    ).toEqual([...MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES].sort());
  });

  it("does not change non-Melbourne merchant feed rows", () => {
  ...
  it("keeps Melbourne listing and governed descriptions free of unrelated destination bleed", () => {
    MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      ...
      expect(governedDescription, productCode).toMatch(/\bMelbourne\b/i);
      expect(cardDescription, productCode).toMatch(/\bMelbourne\b/i);
  ...
  it("keeps Melbourne public itinerary titles clean on rendered detail pages", () => {
...
describe("Melbourne Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified(
      ...
      entry =>
        ...
        MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_MELBOURNE_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Melbourne Engine6 itinerary title governance", () => {
  it("audits all Melbourne listing products", () => {
    expect(melbourneListingTours).toHaveLength(
      MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(melbourneResolvedTours).toHaveLength(
      MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(melbourneResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = melbourneResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
