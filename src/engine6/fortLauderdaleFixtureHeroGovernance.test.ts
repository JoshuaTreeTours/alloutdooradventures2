...
import { NEW_FORT_LAUDERDALE_PRODUCT_CODES } from "./fortLauderdaleViatorPublicRatings";
import { validateEngine6MerchantFeedImageUrl } from "./merchantFeedImageGovernance";

const readFortLauderdaleFixtureHeroUrl = (productCode: string) => {
...
describe("Fort Lauderdale fixture hero governance", () => {
  it("requires hero URLs to originate from live product media images", () => {
    const heroUrl = readFortLauderdaleFixtureHeroUrl("155077P1");
    const payload = {
      product: {
  ...
  it.each(NEW_FORT_LAUDERDALE_PRODUCT_CODES)(
    ...
    async productCode => {
      const heroUrl = readFortLauderdaleFixtureHeroUrl(productCode);
