...
import {
  ...
  AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./amsterdamViatorPublicRatings";
import { ENGINE6_AMSTERDAM_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const amsterdamListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "netherlands" &&
    tour.destination.citySlug === "amsterdam" &&
    AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const amsterdamResolvedTours = engine6ResolvedTours.filter(
  tour =>
...
const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|London|Paris|\bRome\b|\bVenice\b|Washington, D\.C\.|Washington D\.C\./i;

describe("Amsterdam Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        AMSTERDAM_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Amsterdam cohort", () => {
    const amsterdamListing = getToursByCityUnified("netherlands", "amsterdam");
    expect(amsterdamListing).toHaveLength(AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      amsterdamListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(amsterdamListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Amsterdam merchant feed rows", () => {
  ...
  it("keeps Amsterdam listing and governed descriptions free of unrelated destination bleed", () => {
    AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bAmsterdam\b/i);
        expect(cardDescription, productCode).toMatch(/\bAmsterdam\b/i);
  ...
  it("keeps Amsterdam public itinerary titles clean on rendered detail pages", () => {
...
describe("Amsterdam Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_AMSTERDAM_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Amsterdam Engine6 itinerary title governance", () => {
  it("audits all Amsterdam listing products", () => {
    expect(amsterdamListingTours).toHaveLength(
      AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(amsterdamResolvedTours).toHaveLength(
      AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(amsterdamResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(AMSTERDAM_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = amsterdamResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
