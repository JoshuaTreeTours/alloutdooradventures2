...
import { ENGINE6_QUEENSTOWN_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
describe("Queenstown Engine6 rating/review parity", () => {
  ...
  it.each(QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = QUEENSTOWN_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      expect(tour?.canonicalPath).toContain("/new-zealand/queenstown/");
      expect(tour?.state).toBe("New Zealand");
      expect(listingEntry?.tour.destination.state).toBe("New Zealand");
      expect(listingEntry?.tour.destination.stateSlug).toBe("new-zealand");
      expect(listingEntry?.tour.destination.country).toBe("New Zealand");
      expect(listingEntry?.tour.destination.countryCode).toBe("NZ");
      expect(listingEntry?.href).toContain("/new-zealand/queenstown/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("NZ");
      expect(placeNode?.address?.addressRegion).toBe("New Zealand");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("New Zealand");
      ...
      const expectedUsd = QUEENSTOWN_VIATOR_PUBLIC_USD_FROM_PRICES[productCode];
  ...
  it("covers USD From$ for every selected Queenstown product", () => {
    expect(Object.keys(QUEENSTOWN_VIATOR_PUBLIC_USD_FROM_PRICES).sort()).toEqual(
      [...QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("lists exactly the selected Engine6 cards for the Queenstown cohort", () => {
    const queenstownListing = getToursByCityUnified(
      "new-zealand",
      "queenstown"
    );
    expect(queenstownListing).toHaveLength(
      QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      queenstownListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(
      queenstownListing.map(entry => entry.tour.productCode).sort()
    ).toEqual([...QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES].sort());
  });

  it("does not change non-Queenstown merchant feed rows", () => {
  ...
  it("keeps Queenstown listing and governed descriptions free of unrelated destination bleed", () => {
    QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      ...
      expect(governedDescription, productCode).toMatch(/\bQueenstown\b/i);
      expect(cardDescription, productCode).toMatch(/\bQueenstown\b/i);
  ...
  it("keeps Queenstown public itinerary titles clean on rendered detail pages", () => {
...
describe("Queenstown Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified(
      ...
      entry =>
        ...
        QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_QUEENSTOWN_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Queenstown Engine6 itinerary title governance", () => {
  it("audits all Queenstown listing products", () => {
    expect(queenstownListingTours).toHaveLength(
      QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(queenstownResolvedTours).toHaveLength(
      QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(queenstownResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = queenstownResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
