...
const readNaplesFixtureHeroUrl = (productCode: string) => {
...
describe("Naples fixture hero governance", () => {
  it("requires hero URLs to originate from live product media images", () => {
    const heroUrl = readNaplesFixtureHeroUrl("293665P1");
    const payload = {
      product: {
  ...
  it.each(NEW_NAPLES_PRODUCT_CODES)(
    ...
    async productCode => {
      const heroUrl = readNaplesFixtureHeroUrl(productCode);
