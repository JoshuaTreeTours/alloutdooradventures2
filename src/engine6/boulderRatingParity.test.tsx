...
import {
  ...
  BOULDER_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./boulderViatorPublicRatings";
...
const boulderListingTours = engine6ListingTours.filter(
  tour =>
    ...
    BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const boulderResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/colorado/boulder/") &&
    BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion|Yosemite|Glacier National Park|Glacier|Moab|Bryce|Arches|Canyonlands|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Kauai|Waimea Canyon|Na Pali|Great Smoky|Grand Canyon|Hawaii Volcanoes|Kona|Hualalai|Kohala|Aspen|Hunter Creek|Roaring Fork|Wheeler Opera House)/i;

describe("Boulder Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(BOULDER_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = BOULDER_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("colorado", "boulder").find(
  ...
  it("lists exactly the selected Engine6 cards for the Boulder cohort on the Boulder city index", () => {
    const engine6BoulderTours = getToursByCityUnified("colorado", "boulder").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    expect(engine6BoulderTours).toHaveLength(
      BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-Boulder merchant feed rows", () => {
  ...
  it("keeps Boulder listing and governed descriptions free of unrelated destination bleed", () => {
    BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified("colorado", "boulder").find(
      ...
      expect(governedDescription, productCode).toMatch(/\bBoulder\b/i);
      expect(cardDescription, productCode).toMatch(/\bBoulder\b/i);
  ...
  it("keeps Boulder public itinerary titles clean on rendered detail pages", () => {
...
describe("Boulder Engine6 itinerary title governance", () => {
  it("audits all Boulder listing products", () => {
    expect(boulderListingTours).toHaveLength(
      BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(boulderResolvedTours).toHaveLength(
      BOULDER_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(boulderResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...BOULDER_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(BOULDER_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = boulderResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
