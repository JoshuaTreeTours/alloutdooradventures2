...
import {
  ...
  LONDON_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./londonViatorPublicRatings";
import { ENGINE6_LONDON_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const londonListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "united-kingdom" &&
    tour.destination.citySlug === "london" &&
    LONDON_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const londonResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/united-kingdom/london/") &&
    LONDON_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|Washington, D\.C\.|Washington D\.C\./i;

describe("London Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(LONDON_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        LONDON_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the London cohort", () => {
    const engine6LondonTours = getToursByCityUnified(
      "united-kingdom",
      "london"
    ).filter(
      entry =>
    ...
    expect(engine6LondonTours).toHaveLength(
      LONDON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-London merchant feed rows", () => {
  ...
  it("keeps London listing and governed descriptions free of unrelated destination bleed", () => {
    LONDON_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bLondon\b/i);
        expect(cardDescription, productCode).toMatch(/\bLondon\b/i);
  ...
  it("keeps London public itinerary titles clean on rendered detail pages", () => {
...
describe("London Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_LONDON_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("London Engine6 itinerary title governance", () => {
  it("audits all London listing products", () => {
    expect(londonListingTours).toHaveLength(
      LONDON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(londonResolvedTours).toHaveLength(
      LONDON_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(londonResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...LONDON_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(LONDON_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = londonResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
