...
import {
  ...
  ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./orlandoViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_ORLANDO_3170P78_PRODUCT_CODE } from "./routes";
...
const orlandoListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "florida" &&
    tour.destination.citySlug === "orlando" &&
    ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const orlandoResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/florida/orlando/") &&
    ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion|Yosemite|Glacier|Moab|Bryce|Arches|Canyonlands|Key West/i;

describe("Orlando Engine6 rating/review parity", () => {
  ...
  it.each(ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = ORLANDO_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("florida", "orlando").find(
  ...
  it("matches the reported Kennedy Space Center VIP Viator public values", () => {
    expect(
      ORLANDO_VIATOR_PUBLIC_RATINGS[ENGINE6_ORLANDO_3170P78_PRODUCT_CODE]
    ).toEqual({ rating: 5.0, reviewCount: 235 });
  });

  it("lists exactly twenty-two Engine6 cards for the Orlando cohort on the Orlando city index", () => {
    const engine6OrlandoTours = getToursByCityUnified("florida", "orlando").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    expect(engine6OrlandoTours).toHaveLength(22);
  });

  it("does not change non-Orlando merchant feed rows", () => {
  ...
  it("keeps Orlando listing and governed descriptions free of unrelated destination bleed", () => {
    ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified("florida", "orlando").find(
      ...
      expect(governedDescription, productCode).toMatch(/\bOrlando\b/i);
    });
  });

  it("keeps Orlando public itinerary titles clean on rendered detail pages", () => {
...
describe("Orlando Engine6 itinerary title governance", () => {
  it("audits all 22 Orlando listing products", () => {
    expect(orlandoListingTours).toHaveLength(22);
    expect(orlandoResolvedTours).toHaveLength(22);
    expect(orlandoResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(ORLANDO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = orlandoResolvedTours.find(
        entry => entry.productCode === productCode
      );
...
describe("Orlando Engine6 hero diversity governance", () => {
  it("uses unique listing-card heroes across all 22 Orlando products", () => {
    expect(orlandoListingTours).toHaveLength(22);

    const heroCounts = orlandoListingTours.reduce<Map<string, number>>(
      (counts, tour) => {
    ...
    expect(heroCounts.size).toBe(orlandoListingTours.length);
  });
});
