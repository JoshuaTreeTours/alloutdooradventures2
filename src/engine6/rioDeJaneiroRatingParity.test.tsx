...
import {
  ...
  RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./rioDeJaneiroViatorPublicRatings";
import { ENGINE6_RIO_DE_JANEIRO_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const rioDeJaneiroListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "brazil" &&
    tour.destination.citySlug === "rio-de-janeiro" &&
    RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const rioDeJaneiroResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/brazil/rio-de-janeiro/") &&
    RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Rio de Janeiro Engine6 rating/review parity", () => {
  ...
  it.each(RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/brazil/rio-de-janeiro/");
      expect(tour?.state).toBe("Brazil");
      expect(listingEntry?.tour.destination.state).toBe("Brazil");
      expect(listingEntry?.tour.destination.stateSlug).toBe("brazil");
      expect(listingEntry?.tour.destination.country).toBe("Brazil");
      expect(listingEntry?.tour.destination.countryCode).toBe("BR");
      expect(listingEntry?.href).toContain("/brazil/rio-de-janeiro/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("BR");
      expect(placeNode?.address?.addressRegion).toBe("Brazil");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("Brazil");
  ...
  it("lists exactly the selected Engine6 cards for the Rio de Janeiro cohort", () => {
    const rioDeJaneiroListing = getToursByCityUnified("brazil", "rio-de-janeiro");
    expect(rioDeJaneiroListing).toHaveLength(RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      rioDeJaneiroListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(rioDeJaneiroListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Rio de Janeiro merchant feed rows", () => {
  ...
  it("keeps Rio de Janeiro listing and governed descriptions free of unrelated destination bleed", () => {
    RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bRio de Janeiro\b/i);
        expect(cardDescription, productCode).toMatch(/\bRio de Janeiro\b/i);
  ...
  it("keeps Rio de Janeiro public itinerary titles clean on rendered detail pages", () => {
...
describe("Rio de Janeiro Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_RIO_DE_JANEIRO_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Rio de Janeiro Engine6 itinerary title governance", () => {
  it("audits all Rio de Janeiro listing products", () => {
    expect(rioDeJaneiroListingTours).toHaveLength(
      RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(rioDeJaneiroResolvedTours).toHaveLength(
      RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(rioDeJaneiroResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = rioDeJaneiroResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
