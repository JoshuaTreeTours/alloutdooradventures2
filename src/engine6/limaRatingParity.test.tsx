...
import {
  ...
  LIMA_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./limaViatorPublicRatings";
import { ENGINE6_LIMA_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const limaListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "peru" &&
    tour.destination.citySlug === "lima" &&
    LIMA_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const limaResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/peru/lima/") &&
    LIMA_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Lima Engine6 rating/review parity", () => {
  ...
  it.each(LIMA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/peru/lima/");
      ...
      expect(listingEntry?.href).toContain("/peru/lima/");
  ...
  it("lists exactly the selected Engine6 cards for the Lima cohort", () => {
    const limaListing = getToursByCityUnified("peru", "lima");
    expect(limaListing).toHaveLength(LIMA_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      limaListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(limaListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...LIMA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Lima merchant feed rows", () => {
  ...
  it("keeps Lima listing and governed descriptions free of unrelated destination bleed", () => {
    LIMA_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bLima\b/i);
        expect(cardDescription, productCode).toMatch(/\bLima\b/i);
  ...
  it("keeps Lima public itinerary titles clean on rendered detail pages", () => {
...
describe("Lima Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      LIMA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_LIMA_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Lima Engine6 itinerary title governance", () => {
  it("audits all Lima listing products", () => {
    expect(limaListingTours).toHaveLength(
      LIMA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(limaResolvedTours).toHaveLength(
      LIMA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(limaResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...LIMA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(LIMA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = limaResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
