...
import {
  ...
  DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./dublinViatorPublicRatings";
import { ENGINE6_DUBLIN_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const dublinListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "ireland" &&
    tour.destination.citySlug === "dublin" &&
    DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const dublinResolvedTours = engine6ResolvedTours.filter(
  tour =>
...
const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|London|Paris|\bRome\b|\bVenice\b|\bAmsterdam\b|\bBarcelona\b|Washington, D\.C\.|Washington D\.C\./i;

describe("Dublin Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        DUBLIN_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Dublin cohort", () => {
    const dublinListing = getToursByCityUnified("ireland", "dublin");
    expect(dublinListing).toHaveLength(DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      dublinListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(dublinListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Dublin merchant feed rows", () => {
  ...
  it("keeps Dublin listing and governed descriptions free of unrelated destination bleed", () => {
    DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bDublin\b/i);
        expect(cardDescription, productCode).toMatch(/\bDublin\b/i);
  ...
  it("keeps Dublin public itinerary titles clean on rendered detail pages", () => {
...
describe("Dublin Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_DUBLIN_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Dublin Engine6 itinerary title governance", () => {
  it("audits all Dublin listing products", () => {
    expect(dublinListingTours).toHaveLength(
      DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(dublinResolvedTours).toHaveLength(
      DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(dublinResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(DUBLIN_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = dublinResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
