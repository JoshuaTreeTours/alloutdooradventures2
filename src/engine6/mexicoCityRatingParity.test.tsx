...
import {
  ...
  MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./mexicoCityViatorPublicRatings";
import { ENGINE6_MEXICO_CITY_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const mexicoCityListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "mexico" &&
    tour.destination.citySlug === "mexico-city" &&
    MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const mexicoCityResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/mexico/mexico-city/") &&
    MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Mexico City Engine6 rating/review parity", () => {
  ...
  it.each(MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/mexico/mexico-city/");
      ...
      expect(listingEntry?.href).toContain("/mexico/mexico-city/");
  ...
  it("lists exactly the selected Engine6 cards for the Mexico City cohort", () => {
    const mexicoCityListing = getToursByCityUnified("mexico", "mexico-city");
    expect(mexicoCityListing).toHaveLength(MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      mexicoCityListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(mexicoCityListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Mexico City merchant feed rows", () => {
  ...
  it("keeps Mexico City listing and governed descriptions free of unrelated destination bleed", () => {
    MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bMexico City\b/i);
        expect(cardDescription, productCode).toMatch(/\bMexico City\b/i);
  ...
  it("keeps Mexico City public itinerary titles clean on rendered detail pages", () => {
...
describe("Mexico City Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_MEXICO_CITY_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Mexico City Engine6 itinerary title governance", () => {
  it("audits all Mexico City listing products", () => {
    expect(mexicoCityListingTours).toHaveLength(
      MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(mexicoCityResolvedTours).toHaveLength(
      MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(mexicoCityResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = mexicoCityResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
