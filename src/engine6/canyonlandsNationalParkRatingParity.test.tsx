...
import {
  ...
  CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./canyonlandsNationalParkViatorPublicRatings";
import { ENGINE6_CANYONLANDS_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const canyonlandsListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "utah" &&
    tour.destination.citySlug === "canyonlands-national-park" &&
    CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const canyonlandsResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/utah/canyonlands-national-park/") &&
    CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Canyonlands National Park Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Canyonlands cohort", () => {
    const engine6CanyonlandsTours = getToursByCityUnified(
      "utah",
      "canyonlands-national-park"
    ).filter(
      entry =>
    ...
    expect(engine6CanyonlandsTours).toHaveLength(
      CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
  });

  it("does not change non-Canyonlands merchant feed rows", () => {
  ...
  it("keeps Canyonlands listing and governed descriptions free of unrelated destination bleed", () => {
    CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bCanyonlands(?: National Park)?\b/i);
        expect(cardDescription, productCode).toMatch(/\bCanyonlands(?: National Park)?\b/i);
        expect(listingEntry!.tour.shortDescription, productCode).toMatch(
          /\bCanyonlands(?: National Park)?\b/i
        );
      }
    );
  });

  it("keeps Canyonlands public itinerary titles clean on rendered detail pages", () => {
...
describe("Canyonlands National Park Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_CANYONLANDS_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Canyonlands National Park Engine6 itinerary title governance", () => {
  it("audits all Canyonlands listing products", () => {
    expect(canyonlandsListingTours).toHaveLength(
      CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(canyonlandsResolvedTours).toHaveLength(
      CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(canyonlandsResolvedTours.map(tour => tour.productCode).sort()).toEqual(
  ...
  it.each(CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = canyonlandsResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
