...
import {
  ...
  PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./puertoVallartaViatorPublicRatings";
import { ENGINE6_PUERTO_VALLARTA_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const puertoVallartaListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "mexico" &&
    tour.destination.citySlug === "puerto-vallarta" &&
    PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const puertoVallartaResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/mexico/puerto-vallarta/") &&
    PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Puerto Vallarta Engine6 rating/review parity", () => {
  ...
  it.each(PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/mexico/puerto-vallarta/");
      expect(tour?.state).toBe("Mexico");
      expect(listingEntry?.tour.destination.state).toBe("Mexico");
      expect(listingEntry?.tour.destination.stateSlug).toBe("mexico");
      expect(listingEntry?.tour.destination.country).toBe("Mexico");
      expect(listingEntry?.tour.destination.countryCode).toBe("MX");
      expect(listingEntry?.href).toContain("/mexico/puerto-vallarta/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("MX");
      expect(placeNode?.address?.addressRegion).toBe("Mexico");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("Mexico");
  ...
  it("lists exactly the selected Engine6 cards for the Puerto Vallarta cohort", () => {
    const puertoVallartaListing = getToursByCityUnified("mexico", "puerto-vallarta");
    expect(puertoVallartaListing).toHaveLength(PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      puertoVallartaListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(puertoVallartaListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Puerto Vallarta merchant feed rows", () => {
  ...
  it("keeps Puerto Vallarta listing and governed descriptions free of unrelated destination bleed", () => {
    PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bPuerto Vallarta\b/i);
        expect(cardDescription, productCode).toMatch(/\bPuerto Vallarta\b/i);
        expect(listingEntry!.tour.shortDescription, productCode).toMatch(
          /\bPuerto Vallarta\b/i
        );
      }
    );
  });

  it("keeps Puerto Vallarta public itinerary titles clean on rendered detail pages", () => {
...
describe("Puerto Vallarta Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_PUERTO_VALLARTA_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Puerto Vallarta Engine6 itinerary title governance", () => {
  it("audits all Puerto Vallarta listing products", () => {
    expect(puertoVallartaListingTours).toHaveLength(
      PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(puertoVallartaResolvedTours).toHaveLength(
      PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(puertoVallartaResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = puertoVallartaResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
