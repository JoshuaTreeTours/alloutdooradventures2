...
import { OSAKA_VIATOR_PUBLIC_PRODUCT_CODES } from "./osakaViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const osakaFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Osaka Engine6 creation contract", () => {
  it("covers every selected Osaka product code", () => {
    expect(osakaFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...OSAKA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(osakaFixtures)("validates hardened contract for %s", fixture => {
