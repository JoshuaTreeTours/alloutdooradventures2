...
import {
  ...
  DENVER_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./denverViatorPublicRatings";
...
const denverListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "colorado" &&
    tour.destination.citySlug === "denver" &&
    DENVER_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const denverResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/colorado/denver/") &&
    DENVER_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);
...
describe("Denver Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(DENVER_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = DENVER_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("colorado", "denver").find(
  ...
  it("lists exactly sixteen Engine6 cards for the Denver cohort on the Denver city index", () => {
    const engine6DenverTours = getToursByCityUnified("colorado", "denver").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        DENVER_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    expect(engine6DenverTours).toHaveLength(16);
  });

  it("does not change non-Denver merchant feed rows", () => {
  ...
  it("keeps Denver listing and governed descriptions free of unrelated destination bleed", () => {
    DENVER_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified("colorado", "denver").find(
      ...
      expect(governedDescription, productCode).toMatch(/\bDenver\b/i);
      expect(cardDescription, productCode).toMatch(/\bDenver\b/i);
  ...
  it("keeps Denver public itinerary titles clean on rendered detail pages", () => {
...
describe("Denver Engine6 itinerary title governance", () => {
  it("audits all 16 Denver listing products", () => {
    expect(denverListingTours).toHaveLength(16);
    expect(denverResolvedTours).toHaveLength(16);
    expect(denverResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...DENVER_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(DENVER_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = denverResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
