...
import {
  ...
  CUSCO_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./cuscoViatorPublicRatings";
import { ENGINE6_CUSCO_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const caboSanLucasListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "peru" &&
    tour.destination.citySlug === "cusco" &&
    CUSCO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
const caboSanLucasResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/peru/cusco/") &&
    CUSCO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Cusco Engine6 rating/review parity", () => {
  ...
  it.each(CUSCO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/peru/cusco/");
      expect(tour?.state).toBe("Peru");
      expect(listingEntry?.tour.destination.state).toBe("Peru");
      expect(listingEntry?.tour.destination.stateSlug).toBe("peru");
      expect(listingEntry?.tour.destination.country).toBe("Peru");
      expect(listingEntry?.tour.destination.countryCode).toBe("PE");
      expect(listingEntry?.href).toContain("/peru/cusco/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("PE");
      expect(placeNode?.address?.addressRegion).toBe("Peru");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("Peru");
  ...
  it("lists exactly the selected Engine6 cards for the Cusco cohort", () => {
    const caboSanLucasListing = getToursByCityUnified("peru", "cusco");
    expect(caboSanLucasListing).toHaveLength(CUSCO_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
    ...
    expect(caboSanLucasListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...CUSCO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Cusco merchant feed rows", () => {
  ...
  it("keeps Cusco listing and governed descriptions free of unrelated destination bleed", () => {
    CUSCO_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bCusco\b/i);
        expect(cardDescription, productCode).toMatch(/\bCusco\b/i);
  ...
  it("keeps Cusco public itinerary titles clean on rendered detail pages", () => {
...
describe("Cusco Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      CUSCO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_CUSCO_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Cusco Engine6 itinerary title governance", () => {
  it("audits all Cusco listing products", () => {
