...
import { QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES } from "./queenstownViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const queenstownFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Queenstown Engine6 creation contract", () => {
  it("covers every selected Queenstown product code", () => {
    expect(queenstownFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(queenstownFixtures)("validates hardened contract for %s", fixture => {
