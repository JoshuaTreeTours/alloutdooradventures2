...
import { SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES } from "./sydneyViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const sydneyFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Sydney Engine6 creation contract", () => {
  it("covers every selected Sydney product code", () => {
    expect(sydneyFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(sydneyFixtures)("validates hardened contract for %s", fixture => {
