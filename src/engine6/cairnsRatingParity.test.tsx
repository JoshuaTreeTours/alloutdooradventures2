...
import { ENGINE6_CAIRNS_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
describe("Cairns Engine6 rating/review parity", () => {
  ...
  it.each(CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = CAIRNS_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("australia", "cairns").find(
        entry => entry.tour.productCode === productCode
      );
      expect(listingEntry).toBeDefined();
      expect(tour?.canonicalPath).toContain("/australia/cairns/");
      ...
      expect(listingEntry?.href).toContain("/australia/cairns/");
      ...
      const expectedUsd = CAIRNS_VIATOR_PUBLIC_USD_FROM_PRICES[productCode];
  ...
  it("covers USD From$ for every selected Cairns product", () => {
    expect(Object.keys(CAIRNS_VIATOR_PUBLIC_USD_FROM_PRICES).sort()).toEqual(
      [...CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("lists exactly the selected Engine6 cards for the Cairns cohort", () => {
    const cairnsListing = getToursByCityUnified("australia", "cairns");
    expect(cairnsListing).toHaveLength(
      CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      cairnsListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(cairnsListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Cairns merchant feed rows", () => {
  ...
  it("keeps Cairns listing and governed descriptions free of unrelated destination bleed", () => {
    CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      ...
      expect(governedDescription, productCode).toMatch(/\bCairns\b/i);
      expect(cardDescription, productCode).toMatch(/\bCairns\b/i);
  ...
  it("keeps Cairns public itinerary titles clean on rendered detail pages", () => {
...
describe("Cairns Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified("australia", "cairns").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_CAIRNS_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Cairns Engine6 itinerary title governance", () => {
  it("audits all Cairns listing products", () => {
    expect(cairnsListingTours).toHaveLength(
      CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(cairnsResolvedTours).toHaveLength(
      CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(cairnsResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = cairnsResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
