...
import {
  ...
  CANCUN_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./cancunViatorPublicRatings";
import { ENGINE6_CANCUN_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const cancunListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "mexico" &&
    tour.destination.citySlug === "cancun" &&
    CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const cancunResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/mexico/cancun/") &&
    CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Cancun Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(CANCUN_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        CANCUN_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      expect(tour?.canonicalPath).toContain("/mexico/cancun/");
      ...
      expect(listingEntry?.href).toContain("/mexico/cancun/");
  ...
  it("lists exactly the selected Engine6 cards for the Cancun cohort", () => {
    const cancunListing = getToursByCityUnified("mexico", "cancun");
    expect(cancunListing).toHaveLength(CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      cancunListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(cancunListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...CANCUN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Cancun merchant feed rows", () => {
  ...
  it("keeps Cancun listing and governed descriptions free of unrelated destination bleed", () => {
    CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bCancun\b/i);
        expect(cardDescription, productCode).toMatch(/\bCancun\b/i);
  ...
  it("keeps Cancun public itinerary titles clean on rendered detail pages", () => {
...
describe("Cancun Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_CANCUN_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Cancun Engine6 itinerary title governance", () => {
  it("audits all Cancun listing products", () => {
    expect(cancunListingTours).toHaveLength(
      CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(cancunResolvedTours).toHaveLength(
      CANCUN_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(cancunResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...CANCUN_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(CANCUN_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = cancunResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
