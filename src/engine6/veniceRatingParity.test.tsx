...
import {
  ...
  VENICE_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./veniceViatorPublicRatings";
import { ENGINE6_VENICE_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const veniceListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "italy" &&
    tour.destination.citySlug === "venice" &&
    VENICE_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const veniceResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/italy/venice/") &&
    VENICE_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|London|Paris|\bRome\b|Washington, D\.C\.|Washington D\.C\./i;

describe("Venice Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(VENICE_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected =
        VENICE_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("lists exactly the selected Engine6 cards for the Venice cohort", () => {
    const veniceListing = getToursByCityUnified("italy", "venice");
    expect(veniceListing).toHaveLength(VENICE_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      veniceListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(veniceListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...VENICE_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Venice merchant feed rows", () => {
  ...
  it("keeps Venice listing and governed descriptions free of unrelated destination bleed", () => {
    VENICE_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bVenice\b/i);
        expect(cardDescription, productCode).toMatch(/\bVenice\b/i);
  ...
  it("keeps Venice public itinerary titles clean on rendered detail pages", () => {
...
describe("Venice Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      VENICE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_VENICE_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Venice Engine6 itinerary title governance", () => {
  it("audits all Venice listing products", () => {
    expect(veniceListingTours).toHaveLength(
      VENICE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(veniceResolvedTours).toHaveLength(
      VENICE_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(veniceResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...VENICE_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(VENICE_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = veniceResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
