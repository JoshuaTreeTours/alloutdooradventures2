...
import { BARCELONA_VIATOR_PUBLIC_PRODUCT_CODES } from "./barcelonaViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const barcelonaFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  BARCELONA_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Barcelona Engine6 creation contract", () => {
  it("covers every selected Barcelona product code", () => {
    expect(barcelonaFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...BARCELONA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(barcelonaFixtures)("validates hardened contract for %s", fixture => {
