...
import {
  ...
  KONA_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./konaViatorPublicRatings";
...
const konaListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "hawaii" &&
    tour.destination.citySlug === "kona" &&
    KONA_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const konaResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/hawaii/kona/") &&
    KONA_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /\b(?:Chicago|Illinois|Boston|Massachusetts|Philadelphia|Pennsylvania|Miami Beach|Miami|New York|Washington, D\.C\.|Washington D\.C\.|Sedona|Monterey|Napa|Jackson Hole|Wyoming|Yellowstone|Grand Teton|Zion|Yosemite|Glacier|Moab|Bryce|Arches|Canyonlands|Key West|Orlando|Naples|Fort Lauderdale|Maui|Road to Hana|Haleakala|Molokini|Honolulu|Oahu|Pearl Harbor|Waikiki|Kauai|Waimea Canyon|Na Pali|Great Smoky|Grand Canyon|Hawaii Volcanoes)\b/i;

describe("Kona Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(KONA_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = KONA_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("hawaii", "kona").find(
  ...
  it("lists exactly fifteen Engine6 cards for the Kona cohort on the Kona city index", () => {
    const engine6KonaTours = getToursByCityUnified("hawaii", "kona").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        KONA_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    expect(engine6KonaTours).toHaveLength(15);
  });

  it("does not change non-Kona merchant feed rows", () => {
  ...
  it("keeps Kona listing and governed descriptions free of unrelated destination bleed", () => {
    KONA_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified("hawaii", "kona").find(
      ...
      expect(governedDescription, productCode).toMatch(/\bKona\b/i);
      expect(cardDescription, productCode).toMatch(/\bKona\b/i);
  ...
  it("keeps Kona public itinerary titles clean on rendered detail pages", () => {
...
describe("Kona Engine6 itinerary title governance", () => {
  it("audits all 15 Kona listing products", () => {
    expect(konaListingTours).toHaveLength(15);
    expect(konaResolvedTours).toHaveLength(15);
    expect(konaResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...KONA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(KONA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = konaResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
