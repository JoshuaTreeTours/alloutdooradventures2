...
import {
  ...
  ZION_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./zionViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_ZION_265766P27_PRODUCT_CODE } from "./routes";
...
describe("Zion Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(ZION_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = ZION_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("matches the reported small-group tour Viator public values", () => {
    expect(
      ZION_VIATOR_PUBLIC_RATINGS[ENGINE6_ZION_265766P27_PRODUCT_CODE]
    ).toEqual({ rating: 4.9, reviewCount: 77 });
  });

  it("lists exactly seventeen Engine6 cards on the Zion city index", () => {
    const engine6ZionTours = getToursByCityUnified(
      "utah",
      "zion-national-park"
    ).filter(entry => entry.tour.engine === "engine6");
    expect(engine6ZionTours).toHaveLength(17);
  });

  it("does not change non-Zion merchant feed rows", () => {
  ...
  it("keeps Zion public itinerary titles clean on rendered detail pages", () => {
    const malformedTitlePattern =
      /^(?:This|These|That|It|They|inspiration point for photos)$/i;

    ZION_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
