...
import { MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES } from "./melbourneViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const melbourneFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Melbourne Engine6 creation contract", () => {
  it("covers every selected Melbourne product code", () => {
    expect(melbourneFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...MELBOURNE_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(melbourneFixtures)("validates hardened contract for %s", fixture => {
