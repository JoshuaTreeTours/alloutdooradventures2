...
import { ENGINE6_SYDNEY_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
describe("Sydney Engine6 rating/review parity", () => {
  ...
  it.each(SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = SYDNEY_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("australia", "sydney").find(
        entry => entry.tour.productCode === productCode
      );
      expect(listingEntry).toBeDefined();
      expect(tour?.canonicalPath).toContain("/australia/sydney/");
      ...
      expect(listingEntry?.href).toContain("/australia/sydney/");
      ...
      const expectedUsd = SYDNEY_VIATOR_PUBLIC_USD_FROM_PRICES[productCode];
  ...
  it("covers USD From$ for every selected Sydney product", () => {
    expect(Object.keys(SYDNEY_VIATOR_PUBLIC_USD_FROM_PRICES).sort()).toEqual(
      [...SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("lists exactly the selected Engine6 cards for the Sydney cohort", () => {
    const sydneyListing = getToursByCityUnified("australia", "sydney");
    expect(sydneyListing).toHaveLength(
      SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      sydneyListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(sydneyListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Sydney merchant feed rows", () => {
  ...
  it("keeps Sydney listing and governed descriptions free of unrelated destination bleed", () => {
    SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      ...
      expect(governedDescription, productCode).toMatch(/\bSydney\b/i);
      expect(cardDescription, productCode).toMatch(/\bSydney\b/i);
  ...
  it("keeps Sydney public itinerary titles clean on rendered detail pages", () => {
...
describe("Sydney Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified("australia", "sydney").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_SYDNEY_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Sydney Engine6 itinerary title governance", () => {
  it("audits all Sydney listing products", () => {
    expect(sydneyListingTours).toHaveLength(
      SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(sydneyResolvedTours).toHaveLength(
      SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(sydneyResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = sydneyResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
