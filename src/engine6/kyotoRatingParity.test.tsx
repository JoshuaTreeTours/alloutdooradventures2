...
const kyotoListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "japan" &&
    tour.destination.citySlug === "kyoto" &&
    KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);

const kyotoResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/japan/kyoto/") &&
    KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.includes(
      tour.productCode
    )
);
...
describe("Kyoto Engine6 rating/review parity", () => {
  ...
  it.each(KYOTO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      ...
      expect(tour?.canonicalPath).toContain("/japan/kyoto/");
      ...
      expect(listingEntry?.href).toContain("/japan/kyoto/");
  ...
  it("lists exactly the selected Engine6 cards for the Kyoto cohort", () => {
    ...
    expect(kyotoListing).toHaveLength(KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.length);
    expect(
      kyotoListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(kyotoListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...KYOTO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Kyoto merchant feed rows", () => {
  ...
  it("keeps Kyoto listing and governed descriptions free of unrelated destination bleed", () => {
    KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.forEach(
      productCode => {
        const tour = engine6ResolvedTours.find(
          entry => entry.productCode === productCode
        );
        ...
        expect(governedDescription, productCode).toMatch(/\bKyoto\b/i);
        expect(cardDescription, productCode).toMatch(/\bKyoto\b/i);
  ...
  it("keeps Kyoto public itinerary titles clean on rendered detail pages", () => {
...
describe("Kyoto Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    ...
    expect(listingEntries.length).toBe(
      KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_KYOTO_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Kyoto Engine6 itinerary title governance", () => {
  it("audits all Kyoto listing products", () => {
    expect(kyotoListingTours).toHaveLength(
      KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(kyotoResolvedTours).toHaveLength(
      KYOTO_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(kyotoResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...KYOTO_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(KYOTO_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = kyotoResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
