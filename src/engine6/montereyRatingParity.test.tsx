...
import {
  ...
  MONTEREY_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./montereyViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_MONTEREY_CARMEL_WINE_WALK_PRODUCT_CODE } from "./routes";
...
describe("Monterey Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(MONTEREY_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = MONTEREY_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("california", "monterey").find(
  ...
  it("matches the reported Carmel wine walk Viator public values", () => {
    expect(
      MONTEREY_VIATOR_PUBLIC_RATINGS[ENGINE6_MONTEREY_CARMEL_WINE_WALK_PRODUCT_CODE]
    ).toEqual({ rating: 5.0, reviewCount: 74 });
  });

  it("does not change non-Monterey merchant feed rows", () => {
    expect(merchantFeedRows.get("8647P594")).toEqual({
      averageRating: "4.9",
      ratingCount: "95",
      reviewCount: "95",
    });
    expect(merchantFeedRows.size).toBe(177);
  });
});
