...
import { BALI_VIATOR_PUBLIC_PRODUCT_CODES } from "./baliViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const baliFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  BALI_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Bali Engine6 creation contract", () => {
  it("covers every selected Bali product code", () => {
    expect(baliFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...BALI_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(baliFixtures)("validates hardened contract for %s", fixture => {
