...
import {
  ...
  YELLOWSTONE_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./yellowstoneViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_YELLOWSTONE_151830P1_PRODUCT_CODE } from "./routes";
...
describe("Yellowstone Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(YELLOWSTONE_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = YELLOWSTONE_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("matches the reported flagship private tour Viator public values", () => {
    expect(
      YELLOWSTONE_VIATOR_PUBLIC_RATINGS[
        ENGINE6_YELLOWSTONE_151830P1_PRODUCT_CODE
      ]
    ).toEqual({ rating: 5.0, reviewCount: 842 });
  });

  it("lists exactly twenty-three Engine6 cards on the Yellowstone city index", () => {
    const engine6YellowstoneTours = getToursByCityUnified(
      "wyoming",
      "yellowstone-national-park"
    ).filter(entry => entry.tour.engine === "engine6");
    expect(engine6YellowstoneTours).toHaveLength(23);
  });

  it("does not change non-Yellowstone merchant feed rows", () => {
  ...
  it("keeps Yellowstone public itinerary titles clean on rendered detail pages", () => {
    ...
    YELLOWSTONE_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
