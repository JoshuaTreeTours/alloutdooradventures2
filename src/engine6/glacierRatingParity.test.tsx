...
import {
  ...
  GLACIER_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./glacierViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_GLACIER_HALF_DAY_WHITewater_PRODUCT_CODE } from "./routes";
...
describe("Glacier Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(GLACIER_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = GLACIER_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("matches the reported half-day whitewater Viator public values", () => {
    expect(
      GLACIER_VIATOR_PUBLIC_RATINGS[
  ...
  it("keeps Glacier public itinerary titles clean on rendered detail pages", () => {
    const malformedTitlePattern =
      /^(?:This|These|That|It|They|inspiration point for photos)$/i;

    GLACIER_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
