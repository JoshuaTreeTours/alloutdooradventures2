...
import {
  ...
  BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./buenosAiresViatorPublicRatings";
import { ENGINE6_BUENOS_AIRES_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
const buenosAiresListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "argentina" &&
    tour.destination.citySlug === "buenos-aires" &&
    BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const buenosAiresResolvedTours = engine6ResolvedTours.filter(
  tour =>
    tour.canonicalPath.includes("/argentina/buenos-aires/") &&
    BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.includes(tour.productCode)
);

const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Canyonlands|Acadia National Park|Bryce Canyon|Arches National Park|Sedona|Chicago|Boston|\bLondon\b|\bParis\b|\bRome\b|\bVenice\b|Washington, D\.C\.|Washington D\.C\.|\bEdinburgh\b|\bBarcelona\b|\bAmsterdam\b|\bDublin\b|Arthur's Seat|Royal Mile|\bHighlands\b|\bCancun\b|Mexico City|Chichen Itza|Isla Mujeres|Puerto Vallarta|Marietas|Yelapa|Banderas Bay|Cabo San Lucas|El Arco|Land's End|Sea of Cortez|\bCusco\b|\bLima\b|\bPeru\b|Humantay|Machu Picchu|Sacred Valley|Miraflores|Huacachina|Paracas|Pachacamac|Marcahuasi|Iguazu|Iguazú|Colonia del Sacramento/i;

describe("Buenos Aires Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = BUENOS_AIRES_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      expect(tour?.canonicalPath).toContain("/argentina/buenos-aires/");
      expect(tour?.state).toBe("Argentina");
      expect(listingEntry?.tour.destination.state).toBe("Argentina");
      expect(listingEntry?.tour.destination.stateSlug).toBe("argentina");
      expect(listingEntry?.tour.destination.country).toBe("Argentina");
      expect(listingEntry?.tour.destination.countryCode).toBe("AR");
      expect(listingEntry?.href).toContain("/argentina/buenos-aires/");
      ...
      expect(placeNode?.address?.addressCountry).toBe("AR");
      expect(placeNode?.address?.addressRegion).toBe("Argentina");
      expect(breadcrumbNode?.itemListElement?.[1]?.name).toBe("Argentina");
  ...
  it("lists exactly the selected Engine6 cards for the Buenos Aires cohort", () => {
    const buenosAiresListing = getToursByCityUnified(
      "argentina",
      "buenos-aires"
    );
    expect(buenosAiresListing).toHaveLength(
      BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      buenosAiresListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(
      buenosAiresListing.map(entry => entry.tour.productCode).sort()
    ).toEqual([...BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES].sort());
  });

  it("does not change non-Buenos Aires merchant feed rows", () => {
  ...
  it("keeps Buenos Aires listing and governed descriptions free of unrelated destination bleed", () => {
    BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified(
      ...
      expect(governedDescription, productCode).toMatch(/\bBuenos Aires\b/i);
      expect(cardDescription, productCode).toMatch(/\bBuenos Aires\b/i);
  ...
  it("keeps Buenos Aires public itinerary titles clean on rendered detail pages", () => {
...
describe("Buenos Aires Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified(
      ...
      entry =>
        ...
        BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_BUENOS_AIRES_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Buenos Aires Engine6 itinerary title governance", () => {
  it("audits all Buenos Aires listing products", () => {
    expect(buenosAiresListingTours).toHaveLength(
      BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(buenosAiresResolvedTours).toHaveLength(
      BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(buenosAiresResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = buenosAiresResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
