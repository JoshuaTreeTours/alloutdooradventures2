...
import { ENGINE6_OSAKA_CANONICAL_CITY_HERO_URL } from "./displayHero";
...
describe("Osaka Engine6 rating/review parity", () => {
  ...
  it.each(OSAKA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const expected = OSAKA_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("japan", "osaka").find(
        entry => entry.tour.productCode === productCode
      );
      expect(listingEntry).toBeDefined();
      expect(tour?.canonicalPath).toContain("/japan/osaka/");
      ...
      expect(listingEntry?.href).toContain("/japan/osaka/");
      ...
      const expectedUsd = OSAKA_VIATOR_PUBLIC_USD_FROM_PRICES[productCode];
  ...
  it("covers USD From$ for every selected Osaka product", () => {
    expect(Object.keys(OSAKA_VIATOR_PUBLIC_USD_FROM_PRICES).sort()).toEqual(
      [...OSAKA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("lists exactly the selected Engine6 cards for the Osaka cohort", () => {
    const osakaListing = getToursByCityUnified("japan", "osaka");
    expect(osakaListing).toHaveLength(
      OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(
      osakaListing.every(entry => entry.tour.engine === "engine6")
    ).toBe(true);
    expect(osakaListing.map(entry => entry.tour.productCode).sort()).toEqual(
      [...OSAKA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it("does not change non-Osaka merchant feed rows", () => {
  ...
  it("keeps Osaka listing and governed descriptions free of unrelated destination bleed", () => {
    OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      ...
      expect(governedDescription, productCode).toMatch(/\bOsaka\b/i);
      expect(cardDescription, productCode).toMatch(/\bOsaka\b/i);
      expect(listingEntry!.tour.shortDescription, productCode).toMatch(
        /\bOsaka\b/i
      );

      if (productCode !== "20415P6") {
        expect(governedDescription, productCode).not.toMatch(KYOTO_LANDMARK_BLEED);
        expect(cardDescription, productCode).not.toMatch(KYOTO_LANDMARK_BLEED);
      }
    });
  });

  it("keeps Osaka public itinerary titles clean on rendered detail pages", () => {
...
describe("Osaka Engine6 hero diversity", () => {
  it("uses unique listing heroes with at most one canonical city fallback", () => {
    const listingEntries = getToursByCityUnified("japan", "osaka").filter(
      entry =>
        entry.tour.engine === "engine6" &&
        OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.includes(entry.tour.productCode)
    );
    ...
    const canonicalUses = heroes.filter(
      url => url === ENGINE6_OSAKA_CANONICAL_CITY_HERO_URL
    ).length;
    expect(canonicalUses).toBeLessThanOrEqual(1);
  });
});

describe("Osaka Engine6 itinerary title governance", () => {
  it("audits all Osaka listing products", () => {
    expect(osakaListingTours).toHaveLength(
      OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(osakaResolvedTours).toHaveLength(
      OSAKA_VIATOR_PUBLIC_PRODUCT_CODES.length
    );
    expect(osakaResolvedTours.map(tour => tour.productCode).sort()).toEqual(
      [...OSAKA_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });
  ...
  it.each(OSAKA_VIATOR_PUBLIC_PRODUCT_CODES)(
    ...
    productCode => {
      const tour = osakaResolvedTours.find(
        entry => entry.productCode === productCode
      );
      expect(tour).toBeDefined();
