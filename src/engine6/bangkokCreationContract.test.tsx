...
import { BANGKOK_VIATOR_PUBLIC_PRODUCT_CODES } from "./bangkokViatorPublicRatings";

(globalThis as { location?: { pathname: string } }).location = {
  pathname: "/",
};

const bangkokFixtures = ENGINE6_VALIDATION_FIXTURES.filter(fixture =>
  BANGKOK_VIATOR_PUBLIC_PRODUCT_CODES.includes(fixture.productCode)
);
...
describe("Bangkok Engine6 creation contract", () => {
  it("covers every selected Bangkok product code", () => {
    expect(bangkokFixtures.map(fixture => fixture.productCode).sort()).toEqual(
      [...BANGKOK_VIATOR_PUBLIC_PRODUCT_CODES].sort()
    );
  });

  it.each(bangkokFixtures)("validates hardened contract for %s", fixture => {
