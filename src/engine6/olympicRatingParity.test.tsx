...
import {
  ...
  OLYMPIC_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./olympicViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_OLYMPIC_5412OLYM_PRODUCT_CODE } from "./routes";
...
describe("Olympic Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(OLYMPIC_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = OLYMPIC_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("matches the reported flagship small-group tour Viator public values", () => {
    expect(
      OLYMPIC_VIATOR_PUBLIC_RATINGS[ENGINE6_OLYMPIC_5412OLYM_PRODUCT_CODE]
    ).toEqual({ rating: 4.6, reviewCount: 1036 });
  });

  it("lists exactly fourteen Engine6 cards on the Olympic city index", () => {
    const engine6OlympicTours = getToursByCityUnified(
      "washington",
      "olympic-national-park"
    ).filter(entry => entry.tour.engine === "engine6");
    expect(engine6OlympicTours).toHaveLength(14);
  });

  it("does not change non-Olympic merchant feed rows", () => {
  ...
  it("keeps Olympic public itinerary titles clean on rendered detail pages", () => {
    const malformedTitlePattern =
      /^(?:This|These|That|It|They|inspiration point for photos)$/i;

    OLYMPIC_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
