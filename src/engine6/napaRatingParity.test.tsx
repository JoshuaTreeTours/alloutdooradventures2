...
import {
  ...
  NAPA_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./napaViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_NAPA_WINE_TROLLEY_CLASSIC_PRODUCT_CODE } from "./routes";
...
describe("Napa Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(NAPA_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = NAPA_VIATOR_PUBLIC_RATINGS[productCode];
      ...
      const listingEntry = getToursByCityUnified("california", "napa").find(
  ...
  it("matches the reported Napa Wine Trolley Viator public values", () => {
    expect(
      NAPA_VIATOR_PUBLIC_RATINGS[ENGINE6_NAPA_WINE_TROLLEY_CLASSIC_PRODUCT_CODE]
    ).toEqual({ rating: 4.5, reviewCount: 762 });
  });

  it("lists exactly twelve Engine6 cards on the Napa city index", () => {
    const engine6NapaTours = getToursByCityUnified("california", "napa").filter(
      entry => entry.tour.engine === "engine6"
    );
    expect(engine6NapaTours).toHaveLength(12);
  });

  it("does not change non-Napa merchant feed rows", () => {
    expect(merchantFeedRows.get("8647P594")).toEqual({
      averageRating: "4.9",
      ratingCount: "95",
      reviewCount: "95",
    });
    expect(merchantFeedRows.size).toBe(189);
  });
});
