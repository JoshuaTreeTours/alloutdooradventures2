...
import {
  ...
  WASHINGTON_DC_VIATOR_PUBLIC_PRODUCT_CODES,
} from "./washingtonDcViatorPublicRatings";
import { engine6ResolvedTours } from "./registry";
import { ENGINE6_WASHINGTON_DC_67327P4_PRODUCT_CODE } from "./routes";
...
const washingtonDcListingTours = engine6ListingTours.filter(
  tour =>
    tour.engine === "engine6" &&
    tour.destination.stateSlug === "district-of-columbia" &&
    tour.destination.citySlug === "washington"
);

const washingtonDcResolvedTours = engine6ResolvedTours.filter(tour =>
  tour.canonicalPath.includes("/district-of-columbia/washington/")
);

const DESTINATION_BLEED_PATTERN =
  /Yellowstone|Yosemite|Zion National Park|Glacier National Park|Grand Canyon National Park|Great Smoky Mountains|Sedona/i;

describe("Washington D.C. Engine6 rating/review parity", () => {
  const merchantFeedRows = readMerchantFeedRows();

  it.each(WASHINGTON_DC_VIATOR_PUBLIC_PRODUCT_CODES)(
    "keeps %s aligned across registry, listing, detail, schema, and merchant feed",
    productCode => {
      const expected = WASHINGTON_DC_VIATOR_PUBLIC_RATINGS[productCode];
  ...
  it("matches the reported private night monuments Viator public values", () => {
    expect(
      WASHINGTON_DC_VIATOR_PUBLIC_RATINGS[
        ENGINE6_WASHINGTON_DC_67327P4_PRODUCT_CODE
      ]
    ).toEqual({ rating: 5.0, reviewCount: 278 });
  });

  it("lists exactly twenty-two Engine6 cards on the Washington D.C. city index", () => {
    const engine6WashingtonTours = getToursByCityUnified(
      "district-of-columbia",
      "washington"
    ).filter(entry => entry.tour.engine === "engine6");
    expect(engine6WashingtonTours).toHaveLength(22);
  });

  it("does not change non-Washington D.C. merchant feed rows", () => {
  ...
  it("keeps Washington D.C. listing and governed descriptions free of unrelated destination bleed", () => {
    WASHINGTON_DC_VIATOR_PUBLIC_PRODUCT_CODES.forEach(productCode => {
      const tour = engine6ResolvedTours.find(
        entry => entry.productCode === productCode
      );
      const listingEntry = getToursByCityUnified(
      ...
      expect(governedDescription, productCode).toMatch(/\bWashington\b/i);
    });
  });

  it("keeps Washington D.C. public itinerary titles clean on rendered detail pages", () => {
...
describe("Washington D.C. Engine6 itinerary title governance", () => {
  it("audits all 22 Washington D.C. listing products", () => {
    expect(washingtonDcListingTours).toHaveLength(22);
    expect(washingtonDcResolvedTours).toHaveLength(22);
    expect(
      washingtonDcResolvedTours.map(tour => tour.productCode).sort()
    ).toEqual([...WASHINGTON_DC_VIATOR_PUBLIC_PRODUCT_CODES].sort());
  });

  it.each(WASHINGTON_DC_VIATOR_PUBLIC_PRODUCT_CODES)(
    "uses verified POI titles for resolved tour %s",
    productCode => {
      const tour = washingtonDcResolvedTours.find(
        entry => entry.productCode === productCode
      );
...
describe("Washington D.C. Engine6 hero diversity governance", () => {
  it("uses unique listing-card heroes across all 22 Washington D.C. products", () => {
    expect(washingtonDcListingTours).toHaveLength(22);

    const heroCounts = washingtonDcListingTours.reduce<Map<string, number>>(
      (counts, tour) => {
    ...
    expect(heroCounts.size).toBe(washingtonDcListingTours.length);
  });
});
