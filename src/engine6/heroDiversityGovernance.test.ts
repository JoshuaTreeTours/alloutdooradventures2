...
import {
  ...
  ENGINE6_KONA_CANONICAL_CITY_HERO_URL,
...
describe("Engine6 hero diversity governance", () => {
  ...
  it("uses Kona as the validation cohort for unique listing-card heroes", () => {
    expect(konaListingTours.length).toBeGreaterThan(0);

    const heroCounts = konaListingTours.reduce<Map<string, number>>(
      (counts, tour) => {
        counts.set(tour.heroImage, (counts.get(tour.heroImage) ?? 0) + 1);
        return counts;
      },
    ...
    expect(
      heroCounts.get(ENGINE6_KONA_CANONICAL_CITY_HERO_URL) ?? 0
    ).toBeLessThanOrEqual(1);
    expect(heroCounts.size).toBe(konaListingTours.length);
  });

  it("uses Denver as the validation cohort for unique listing-card heroes", () => {
    expect(denverListingTours.length).toBeGreaterThan(0);

    const heroCounts = denverListingTours.reduce<Map<string, number>>(
      (counts, tour) => {
        counts.set(tour.heroImage, (counts.get(tour.heroImage) ?? 0) + 1);
        return counts;
      },
    ...
    expect(
      heroCounts.get(ENGINE6_DENVER_CANONICAL_CITY_HERO_URL) ?? 0
    ).toBeLessThanOrEqual(1);
    expect(heroCounts.size).toBe(denverListingTours.length);
  });

  it("uses Aspen as the validation cohort for unique listing-card heroes", () => {
    expect(aspenListingTours.length).toBeGreaterThan(0);

    const heroCounts = aspenListingTours.reduce<Map<string, number>>(
      (counts, tour) => {
        counts.set(tour.heroImage, (counts.get(tour.heroImage) ?? 0) + 1);
        return counts;
      },
