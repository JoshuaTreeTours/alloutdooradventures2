...
export default function GuidePageTemplate({ guide }: GuidePageTemplateProps) {
  ...
  const seoDescription = guide.city
    ? `Plan your ${guide.city} trip with travel tips, top attractions, outdoor activities, and curated local experiences.`
    : guide.overview[0];
  ...
  const allCityTourEntries = useMemo(
    () => allCityTours.map(tour => ({ tour })),
    [allCityTours]
  );
  ...
  const hydratedAllCityTours = useMemo(
    ...
    [allCityTourEntries, liveEngine6DynamicByProductCode]
  );
      ...
      <section className="mx-auto max-w-6xl px-6 py-14">
        ...
        {guide.tours.citySlug ? (
          <AvailableToursGrid
            ...
            citySlug={guide.tours.citySlug}
            stateSlug={guide.tours.stateSlug}
            tours={hydratedAllCityTours.map(entry => entry.tour)}
          />
        ) : null}
      </section>
    </main>
