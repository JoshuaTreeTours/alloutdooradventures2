...
import { oregonRows } from "./oregon.rows";
...
export const loadOregonEngine2Tours = (): Engine2Tour[] => {
  ...
  for (const row of oregonRows) {
    ...
    if (slugify(countryRaw) !== "united-states" || slugify(regionRaw) !== "oregon") {
      continue;
    }
    ...
    byId.set(id, {
      ...
      seo: {
        title: `${name} | ${toTitleCase(cityRaw)}, Oregon Tour`,
