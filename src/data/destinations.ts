...
export const states: StateDestination[] = rawStates.map((state) => {
  const normalizedCities = state.cities.map((city) => {
    const fallbackImages = buildCityHeroImages(
      state.slug,
      city.slug,
      city.heroImages?.length ?? 3
    );

    return {
      ...city,
      heroImages: normalizeImageArray(city.heroImages, fallbackImages),
    };
  });

  return {
    ...state,
    heroImage: normalizeImagePath(state.heroImage, buildStateHeroImage(state.slug)),
...
if (import.meta.env?.DEV) {
  ...
  const citySummary = states.flatMap((state) =>
    state.cities.map((city) => ({
      slug: `${state.slug}/${city.slug}`,
      images: city.heroImages,
    }))
  );
  const missingStateImages = stateSummary.filter(
    (state) => state.image === PLACEHOLDER_IMAGE
  );
  const missingCityImages = citySummary.filter((city) =>
    city.images.every((image) => image === PLACEHOLDER_IMAGE)
  );

  console.warn("[image-audit] State hero images:", stateSummary);
  console.warn("[image-audit] City hero images:", citySummary);

  if (missingStateImages.length > 0) {
    console.warn("[image-audit] Missing state hero images:", missingStateImages);
  }

  if (missingCityImages.length > 0) {
    console.warn("[image-audit] Missing city hero images:", missingCityImages);
  }
}
