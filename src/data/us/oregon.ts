...
export function loadOregonTours(): OregonTour[] {
  const filePath = path.join(process.cwd(), "data/oregon.csv");

  if (!fs.existsSync(filePath)) {
    console.warn("⚠️ oregon.csv not found");
    return [];
  }
  ...
  return records
    ...
    .filter((tour): tour is OregonTour => Boolean(tour));
}
