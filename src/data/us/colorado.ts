...
export function loadColoradoTours(): ColoradoTour[] {
  const filePath = path.join(process.cwd(), "data/heartland/colorado.csv");

  if (!fs.existsSync(filePath)) {
    console.warn("⚠️ data/heartland/colorado.csv not found");
    return [];
  }
  ...
  return records
    ...
    .filter((tour): tour is ColoradoTour => Boolean(tour));
}
