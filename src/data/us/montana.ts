...
export function loadMontanaTours(): MontanaTour[] {
  const filePath = path.join(process.cwd(), "data/heartland/montana.csv");

  if (!fs.existsSync(filePath)) {
    console.warn("⚠️ data/heartland/montana.csv not found");
    return [];
  }
  ...
  return records
    ...
    .filter((tour): tour is MontanaTour => Boolean(tour));
}
