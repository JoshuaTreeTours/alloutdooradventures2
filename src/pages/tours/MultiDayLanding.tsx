...
const getStableTourKey = (tour: Tour) => {
  const itemId = tour.bookingUrl.match(/\/items\/(\d+)/)?.[1];
  if (itemId) return `fh:${itemId}`;
  if (tour.productCode) return `viator:${tour.productCode}`;
  return `tour:${tour.id}`;
};
...
export default function MultiDayLanding() {
  ...
  const multiDayTours = useMemo(() => {
    ...
    const merged = [...tours, ...engine2International];
    const deduped = new Map<string, Tour>();
    merged.forEach(tour => deduped.set(getStableTourKey(tour), tour));
    return [...deduped.values()].filter(tour =>
  ...
  const worldOptions = useMemo(() => {
    ...
    multiDayTours.forEach(tour => {
      ...
      if (
        usStateMap.has(slug) ||
        europeMap.has(slug) ||
        AFRICA_COUNTRY_SLUGS.has(slug)
      ) {
        return;
      }
      if (
        slug === "united-states" ||
        (tour.destination.country || "").toLowerCase() === "united states"
      ) {
        return;
      }
      if (!destinationMap.has(slug)) {
  ...
  const selectedSlug =
    selectedState || selectedEurope || selectedAfrica || selectedWorld;
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-16 text-[#1f2a1f]">
      ...
      <p className="mt-4 max-w-3xl text-sm text-[#405040] md:text-base">
        ...
        Select a state or international destination to see available tours.
      </p>
      ...
      <section className="mt-8 rounded-2xl border border-black/10 bg-white/70 p-6 text-sm text-[#405040]">
        {hasSelection ? (
          ...
          <p>
            Choose a state or international destination to see multi-day tours.
          </p>
        )}
      </section>
