...
const codes = [...CANCUN_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/mexico/cancun/")) {
    issues.push("bad path");
  }
  if (!/Cancun/i.test(governed)) {
    issues.push("no Cancun in governed");
  }
  if (!/Cancun/i.test(card)) {
    issues.push("no Cancun in card excerpt");
  }
...
writeFileSync(
  "scripts/cancun-render-surface-report.json",
