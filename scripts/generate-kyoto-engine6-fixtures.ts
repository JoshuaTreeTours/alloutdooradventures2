/**
 * Generates Kyoto Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-kyoto-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { KYOTO_VIATOR_PUBLIC_RATINGS } from "../src/engine6/kyotoViatorPublicRatings";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    "Ideal for visitors basing in Kyoto who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Kyoto sightseeing experience, with a guide handling hotel pickup, boat connections, and mountain or island pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "Kinkakuji Temple", stopType: "stop" as const },
      {
        title:
          first && first !== "Kinkakuji Temple"
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    KYOTO_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Kyoto`,
            "Professional Kyoto guide",
          ].slice(0, 5)
        : [
            `${live.title} from Kyoto`,
            "Kinkaku-ji, Fushimi Inari, and Gion views",
            "Professional guide with Kyoto local expertise",
            "Small-group or private format as listed",
            "Guided Kyoto sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Kyoto meeting location listed when booking.",
    endDescription:
      "Return to your Kyoto meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    KYOTO_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Kyoto?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Kyoto Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    destinationCitySlug: "kyoto",
    stateSlug: "japan",
    citySlug: "kyoto",
    viatorDestinationSlug: "Kyoto",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Kyoto",
  });
};
