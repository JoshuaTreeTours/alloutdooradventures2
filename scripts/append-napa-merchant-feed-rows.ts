...
const newRows = NAPA_VIATOR_PUBLIC_PRODUCT_CODES.map(productCode => {
  if (existingIds.has(productCode)) {
    throw new Error(`Refusing to overwrite existing merchant feed row ${productCode}`);
  }
  ...
  if (!tour) {
    throw new Error(`Missing resolved Napa tour for ${productCode}`);
  }
...
console.log(
  `Appended ${newRows.length} Napa merchant feed rows (${existingLines.length - 1} -> ${existingLines.length - 1 + newRows.length}).`
);
