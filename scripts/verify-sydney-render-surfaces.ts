...
const codes = [...SYDNEY_VIATOR_PUBLIC_PRODUCT_CODES];
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/australia/sydney/")) {
    issues.push("bad path");
  }
  if (!/Sydney/i.test(governed)) {
    issues.push("no Sydney in governed");
  }
  if (!/Sydney/i.test(card)) {
    issues.push("no Sydney in card excerpt");
  }
  ...
  const expectedUsd = SYDNEY_VIATOR_PUBLIC_USD_FROM_PRICES[code];
  const expectedRating = SYDNEY_VIATOR_PUBLIC_RATINGS[code];
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/australia/sydney/"))
...
writeFileSync(
  "scripts/sydney-render-surface-report.json",
