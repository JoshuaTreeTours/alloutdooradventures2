/**
 * Generates and applies Dublin Engine6 wiring.
 * Run: npx tsx scripts/generate-dublin-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/dublinViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/dublin-product-catalog.json";
...
const NARRATIVES_PATH =
  "src/engine6/dublinApprovedNarrativeDescriptions.ts";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_DUBLIN_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type DublinViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
for (const product of products) {
  ...
  if (!/\bDublin\b/i.test(text.slice(0, 150))) {
    throw new Error(
      `Approved narrative for ${product.productCode} must name Dublin in the first 150 characters`
    );
  }
...
const narrativesTs = `export const DUBLIN_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;
...
if (
  !routesContent.includes(
    "ENGINE6_DUBLIN_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Dublin route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_DUBLIN_103208P1_ROUTE,")) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Dublin route map entries.`
  );
} else {
  console.log(
    "Dublin route map entries already present."
  );
}
...
if (!fixturesContent.includes('productCode: "103208P1"')) {
  ...
  console.log(
    `Applied ${products.length} Dublin validation fixture imports and entries.`
  );
} else {
  console.log(
    "Dublin validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Dublin products.`
);
