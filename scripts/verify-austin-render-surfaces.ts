...
import { AUSTIN_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/austinViatorPublicRatings";
import {
...
const codes = [...AUSTIN_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/texas/austin/")) {
    issues.push("bad path");
  }
  if (!/Austin/i.test(governed)) issues.push("no Austin in governed");
  if (!/Austin/i.test(card)) issues.push("no Austin in card excerpt");
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/texas/austin/"))
