/**
 * Generates and applies Osaka Engine6 wiring.
 * Run: npx tsx scripts/generate-osaka-engine6-wiring.ts
 */
...
const CATALOG_PATH = "scripts/osaka-product-catalog.json";
const LIVE_DATA_PATH = "scripts/osaka-live-product-data.json";
const SELECTION_PATH = "scripts/osaka-product-selection.json";
const APPROVED_NARRATIVES_JSON = "scripts/osaka-approved-narratives.json";
const EDITORIAL_OVERRIDES_PATH = "scripts/osaka-editorial-overrides.json";

const ROUTE_PREFIX = "/destinations/japan/osaka/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_SEOUL_42053P19_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_OSAKA_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
if (!routesContent.includes("ENGINE6_OSAKA_TOUR_PATH_PREFIX")) {
  if (!routesContent.includes(ROUTES_INSERT_BEFORE)) {
    throw new Error("Missing Bali route constant anchor in routes.ts");
  }
  ...
  console.log(`Applied ${products.length} Osaka route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (
  !routesContent.includes(
    `ENGINE6_OSAKA_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`
  )
) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Bali route map anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Osaka route map entries.`);
} else {
  console.log("Osaka route map entries already present.");
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  ...
  console.log(
    `Applied ${products.length} Osaka validation fixture imports and entries.`
  );
} else {
  console.log("Osaka validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Osaka products.`);
