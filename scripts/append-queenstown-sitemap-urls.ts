...
import catalog from "./queenstown-product-catalog.json";
...
if (newUrls.length === 0) {
  console.log("Sitemap already contains Queenstown tour URLs.");
  process.exit(0);
}

const closingTag = "</urlset>";
const next = existing.replace(closingTag, `${newUrls.join("\n")}\n${closingTag}`);
writeFileSync(sitemapPath, next, "utf8");
console.log(`Appended ${newUrls.length} Queenstown sitemap URLs.`);
