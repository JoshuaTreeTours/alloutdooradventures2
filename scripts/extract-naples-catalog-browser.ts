/**
 * Playwright extraction of Naples, Florida Viator catalog product links.
 * Run: npx tsx scripts/extract-naples-catalog-browser.ts
 */
import { writeFileSync } from "node:fs";

import { chromium } from "playwright";

const CATALOG_URL = "https://www.viator.com/Naples/d22381-ttd";

const EXTRACT_CATALOG_JS = `(() => {
  const html = document.documentElement.innerHTML;
  const products = new Map();
  for (const match of html.matchAll(/https:\\/\\/www\\.viator\\.com\\/tours\\/Naples\\/[^"'\\s]+?\\/d22381-([A-Z0-9_]+)/gi)) {
    products.set(match[1].toUpperCase(), match[0].split('?')[0]);
  }
  for (const match of html.matchAll(/\\/tours\\/Naples\\/[^"'\\s]+?\\/d22381-([A-Z0-9_]+)/gi)) {
    const code = match[1].toUpperCase();
    if (!products.has(code)) products.set(code, 'https://www.viator.com' + match[0].split('?')[0]);
  }
  for (const a of document.querySelectorAll('a[href*="d22381-"]')) {
    const href = a.href || a.getAttribute('href') || '';
    const m = href.match(/d22381-([A-Z0-9_]+)/i);
    if (m) {
...
const main = async () => {
  ...
  writeFileSync(
    "scripts/naples-catalog-products.json",
