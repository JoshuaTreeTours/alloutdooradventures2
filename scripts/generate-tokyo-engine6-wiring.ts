/**
 * Generates and applies Tokyo Engine6 wiring.
 * Run: npx tsx scripts/generate-tokyo-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/tokyoViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/tokyo-product-catalog.json";
...
const NARRATIVES_PATH =
  "src/engine6/tokyoApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/tokyo-approved-narratives.json";

const ROUTE_PREFIX =
  "/destinations/japan/tokyo/tours/";
const ROUTES_INSERT_BEFORE =
  "export const ENGINE6_RIO_DE_JANEIRO_20297P1_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_TOKYO_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    experienceType.includes("wine") || experienceType.includes("tequila")
      ? "Ideal for visitors basing in Tokyo who want a guided tasting day without coordinating rural driving or tasting-room appointments on their own."
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ? "Ideal for visitors basing in Tokyo who want a guided off-road day without renting ATVs independently."
        : experienceType.includes("bike")
          ? "Ideal for visitors basing in Tokyo who want a guided bike outing covering the coast without renting independently."
          : "Ideal for visitors basing in Tokyo who want a guided bay or mountain experience without coordinating tickets or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    const logistics =
      experienceType.includes("wine") || experienceType.includes("tequila")
        ? "with a guide handling tasting timing, ranch pacing, and return transfers to Tokyo"
        : experienceType.includes("jeep") || experienceType.includes("offroad")
          ? "with a guide handling ATV briefing, jungle routing, and return to Tokyo"
          : experienceType.includes("bike")
            ? "with a cycling guide handling bikes, coastal pacing, and Meiji Jingu routing"
            : "with a guide handling ticket windows, coach or boat connections, and neighborhood pacing";
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Tokyo experience, ${logistics} so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const ratingsTs = `export type TokyoViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const TOKYO_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type TokyoTargetedNarrativeDescriptionProductCode =
  (typeof TOKYO_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const TOKYO_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  TokyoTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getTokyoTargetedNarrativeDescription = (
  productCode: string
) =>
  TOKYO_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as TokyoTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (
  !routesContent.includes(
    "ENGINE6_TOKYO_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Tokyo route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes(`ENGINE6_TOKYO_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`)) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Tokyo route map entries.`
  );
} else {
  console.log(
    "Tokyo route map entries already present."
  );
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  ...
  console.log(
    `Applied ${products.length} Tokyo validation fixture imports and entries.`
  );
} else {
  console.log(
    "Tokyo validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Tokyo products.`
);
