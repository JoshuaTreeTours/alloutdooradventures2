/**
 * Generates Osaka Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-osaka-engine6-fixtures.ts --bootstrap
 */
...
} from "../src/engine6/osakaViatorPublicRatings";
...
const LIVE_DATA_PATH = "scripts/osaka-live-product-data.json";
const EDITORIAL_OVERRIDES_PATH = "scripts/osaka-editorial-overrides.json";
const DESTINATION_CITY = "Osaka";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    throw new Error(
      `Osaka product ${live.productCode} needs at least two verified itinerary stops`
    );
  }
...
const buildTourFromLive = (live: LiveProduct): OsakaTourFixture => {
  const usdPrice = OSAKA_VIATOR_PUBLIC_USD_FROM_PRICES[live.productCode];
  const priceFrom = usdPrice ?? parsePrice(live.priceFrom);
  if (!usdPrice || priceFrom !== usdPrice) {
    throw new Error(
      `Osaka product ${live.productCode} is missing a verified USD From price`
    );
  }
  const rating =
    live.rating ??
    OSAKA_VIATOR_PUBLIC_RATINGS[live.productCode]?.rating ??
    null;
  const reviewCount =
    live.reviewCount ??
    OSAKA_VIATOR_PUBLIC_RATINGS[live.productCode]?.reviewCount ??
    0;
  if (rating == null) {
    throw new Error(
      `Osaka product ${live.productCode} is missing a verified rating`
    );
  }
  ...
  if (!description) {
    throw new Error(
      `Osaka product ${live.productCode} is missing an approved narrative`
    );
  }
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Osaka meeting location listed when booking.",
    endDescription:
      "Return to your Osaka meeting point after the final stop on the itinerary.",
...
const SELECTION_PATH = "scripts/osaka-product-selection.json";
...
const OSAKA_TOURS: OsakaTourFixture[] = liveProducts.map(buildTourFromLive);

const buildFixture = (tour: OsakaTourFixture) => {
  const viatorRatings = OSAKA_VIATOR_PUBLIC_RATINGS[tour.productCode];
  const rating = viatorRatings?.rating ?? tour.rating;
  const reviewCount = viatorRatings?.reviewCount ?? tour.reviewCount;
  const usdPrice = OSAKA_VIATOR_PUBLIC_USD_FROM_PRICES[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question: "Where does the tour depart from for Osaka?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(`Bootstrapped ${OSAKA_TOURS.length} Osaka Engine6 fixtures.`);
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "japan",
    citySlug: "osaka",
    viatorDestinationSlug: "Osaka",
    targetPremiumShare: 0.3,
    tours: OSAKA_TOURS,
    buildFixture,
    destinationLogLabel: "Osaka",
  });
};
