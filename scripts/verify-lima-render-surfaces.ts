...
const codes = [...LIMA_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/peru/lima/")) {
    issues.push("bad path");
  }
  if (!/Lima/i.test(governed)) {
    issues.push("no Lima in governed");
  }
  if (!/Lima/i.test(card)) {
    issues.push("no Lima in card excerpt");
  }
...
writeFileSync(
  "scripts/lima-render-surface-report.json",
