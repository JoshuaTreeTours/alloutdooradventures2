/**
 * One-off discovery: live Viator public page data for Honolulu catalog.
 * Run: npx tsx scripts/discover-honolulu-viator-catalog.ts
 */
...
const CATALOG_URL = "https://www.viator.com/Honolulu/d59070-ttd";
const VIATOR_DESTINATION_ID = "d59070";
...
const extractCatalogProducts = (html: string) => {
  ...
  for (const match of html.matchAll(
    new RegExp(
      `https:\\/\\/www\\.viator\\.com\\/tours\\/Honolulu\\/[^"'\\s]+?\\/${VIATOR_DESTINATION_ID}-([A-Z0-9_]+)`,
      "gi"
    )
  )) {
  ...
  for (const match of html.matchAll(
    new RegExp(
      `/tours/Honolulu/[^"'\\s]+?/${VIATOR_DESTINATION_ID}-([A-Z0-9_]+)`,
      "gi"
    )
  )) {
...
const PREFERRED_PATTERN =
  /private|premium|cultural|pearl harbor|north shore|food|hike|hiking|ocean|volcano|geology|histor|photograph|day trip|circle island|snorkel|luau|helicopter|sunset|sunrise|diamond head|waikiki|oahu|kualoa|polynesian|memorial|uss arizona|luau|catamaran|whale|turtle|kayak|surf|waterfall|rainforest|botanical|museum|monument|scenic|adventure|small group|exclusive|luxury/i;
...
const main = async () => {
  ...
  results.sort((a, b) => {
    ...
    const priceA = (a.priceFrom as number) ?? 0;
    const priceB = (b.priceFrom as number) ?? 0;
    if (priceB !== priceA) {
  ...
  writeFileSync(
    "scripts/honolulu-catalog-products.json",
    `${JSON.stringify(
      results.map(r => ({
