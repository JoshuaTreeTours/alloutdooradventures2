...
const codes = [...QUEENSTOWN_VIATOR_PUBLIC_PRODUCT_CODES];
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/new-zealand/queenstown/")) {
    issues.push("bad path");
  }
  if (!/Queenstown/i.test(governed)) {
    issues.push("no Queenstown in governed");
  }
  if (!/Queenstown/i.test(card)) {
    issues.push("no Queenstown in card excerpt");
  }
  ...
  const expectedUsd = QUEENSTOWN_VIATOR_PUBLIC_USD_FROM_PRICES[code];
  const expectedRating = QUEENSTOWN_VIATOR_PUBLIC_RATINGS[code];
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/new-zealand/queenstown/"))
...
writeFileSync(
  "scripts/queenstown-render-surface-report.json",
