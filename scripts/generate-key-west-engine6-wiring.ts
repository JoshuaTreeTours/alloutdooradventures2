/**
 * Generates and applies Key West Engine6 wiring from key-west-live-product-data.json
 * Run: npx tsx scripts/generate-key-west-engine6-wiring.ts
 */
...
const RATINGS_PATH = "src/engine6/keyWestViatorPublicRatings.ts";
const CATALOG_PATH = "scripts/key-west-product-catalog.json";
const LIVE_DATA_PATH = "scripts/key-west-live-product-data.json";

const ROUTE_PREFIX = "/destinations/florida/key-west/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_MOAB_TOUR_PATH_PREFIX";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_KEY_WEST_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type KeyWestViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
if (!routesContent.includes("ENGINE6_KEY_WEST_TOUR_PATH_PREFIX")) {
  routesContent = routesContent.replace(
    ROUTES_INSERT_BEFORE,
    `${routeBlock}${ROUTES_INSERT_BEFORE}`
  );
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Key West route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_KEY_WEST_331502P3_ROUTE,")) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Moab route anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Key West route map entries.`);
} else {
  console.log("Key West route map entries already present.");
}
...
if (!fixturesContent.includes('productCode: "331502P3"')) {
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error("Missing Moab fixture anchor in validationFixtures.ts");
  }
  ...
  console.log(
    `Applied ${products.length} Key West validation fixture imports and entries.`
  );
} else {
  console.log("Key West validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Key West products.`);
