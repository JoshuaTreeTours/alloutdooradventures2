/**
 * Generates and applies Kona Engine6 wiring from kona-live-product-data.json
 * Run: npx tsx scripts/generate-kona-engine6-wiring.ts
 */
...
const RATINGS_PATH = "src/engine6/konaViatorPublicRatings.ts";
const CATALOG_PATH = "scripts/kona-product-catalog.json";
const LIVE_DATA_PATH = "scripts/kona-live-product-data.json";
const NARRATIVES_PATH = "src/engine6/konaApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON = "scripts/kona-approved-narratives.json";

const ROUTE_PREFIX = "/destinations/hawaii/kona/tours/";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_KONA_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type KonaViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const KONA_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type KonaTargetedNarrativeDescriptionProductCode =
  (typeof KONA_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const KONA_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  KonaTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getKonaTargetedNarrativeDescription = (productCode: string) =>
  KONA_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as KonaTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (!routesContent.includes("ENGINE6_KONA_TOUR_PATH_PREFIX")) {
  routesContent = routesContent.replace(
    ROUTES_INSERT_BEFORE,
    `${routeBlock}${ROUTES_INSERT_BEFORE}`
  );
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Kona route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_KONA_28456P13_ROUTE,")) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Kauai route anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Kona route map entries.`);
} else {
  console.log("Kona route map entries already present.");
}
...
if (!fixturesContent.includes('productCode: "28456P13"')) {
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error("Missing Kauai fixture anchor in validationFixtures.ts");
  }
  ...
  console.log(
    `Applied ${products.length} Kona validation fixture imports and entries.`
  );
} else {
  console.log("Kona validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Kona products.`);
