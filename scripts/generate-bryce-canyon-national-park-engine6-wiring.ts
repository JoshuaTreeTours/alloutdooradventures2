/**
 * Generates and applies Bryce Canyon National Park Engine6 wiring.
 * Run: npx tsx scripts/generate-bryce-canyon-national-park-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/bryceCanyonNationalParkViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/bryce-canyon-national-park-product-catalog.json";
...
const NARRATIVES_PATH =
  "src/engine6/bryceCanyonNationalParkApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/bryce-canyon-national-park-approved-narratives.json";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_BRYCE_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type BryceCanyonNationalParkViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const BRYCE_CANYON_NATIONAL_PARK_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type BryceCanyonNationalParkTargetedNarrativeDescriptionProductCode =
  (typeof BRYCE_CANYON_NATIONAL_PARK_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const BRYCE_CANYON_NATIONAL_PARK_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  BryceCanyonNationalParkTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getBryceCanyonNationalParkTargetedNarrativeDescription = (
  productCode: string
) =>
  BRYCE_CANYON_NATIONAL_PARK_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as BryceCanyonNationalParkTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (
  !routesContent.includes(
    "ENGINE6_BRYCE_CANYON_NATIONAL_PARK_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Bryce Canyon National Park route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_BRYCE_165275P1_ROUTE,")) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Bryce Canyon National Park route map entries.`
  );
} else {
  console.log(
    "Bryce Canyon National Park route map entries already present."
  );
}
...
if (!fixturesContent.includes('productCode: "165275P1"')) {
  if (!fixturesContent.includes(FIXTURE_IMPORTS_INSERT_BEFORE)) {
    throw new Error(
      "Missing Austin fixture import anchor in validationFixtures.ts"
    );
  }
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error(
      "Missing Austin fixture entry anchor in validationFixtures.ts"
    );
  }
  ...
  console.log(
    `Applied ${products.length} Bryce Canyon National Park validation fixture imports and entries.`
  );
} else {
  console.log(
    "Bryce Canyon National Park validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Bryce Canyon National Park products.`
);
