/**
 * Generates and applies Cairns Engine6 wiring.
 * Run: npx tsx scripts/generate-cairns-engine6-wiring.ts
 */
...
const APPROVED_NARRATIVES_JSON = "scripts/cairns-approved-narratives.json";
const EDITORIAL_OVERRIDES_PATH = "scripts/cairns-editorial-overrides.json";

const ROUTE_PREFIX = "/destinations/australia/cairns/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_SYDNEY_184156P4_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_CAIRNS_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
if (!routesContent.includes("ENGINE6_CAIRNS_TOUR_PATH_PREFIX")) {
  if (!routesContent.includes(ROUTES_INSERT_BEFORE)) {
    throw new Error("Missing Sydney route constant anchor in routes.ts");
  }
  ...
  console.log(`Applied ${products.length} Cairns route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (
  !routesContent.includes(
    `ENGINE6_CAIRNS_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`
  )
) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Sydney route map anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Cairns route map entries.`);
} else {
  console.log("Cairns route map entries already present.");
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  if (!fixturesContent.includes(FIXTURE_IMPORTS_INSERT_BEFORE)) {
    throw new Error(
      "Missing Sydney fixture import anchor in validationFixtures.ts"
    );
  }
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error(
      "Missing Sydney fixture entry anchor in validationFixtures.ts"
    );
  }
  ...
  console.log(
    `Applied ${products.length} Cairns validation fixture imports and entries.`
  );
} else {
  console.log("Cairns validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Cairns products.`);
