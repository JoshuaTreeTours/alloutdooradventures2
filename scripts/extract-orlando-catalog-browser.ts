/**
 * Playwright extraction of Orlando Viator catalog product links.
 * Run: npx tsx scripts/extract-orlando-catalog-browser.ts
 */
...
const EXTRACT_CATALOG_JS = `(() => {
  ...
  for (const match of html.matchAll(/https:\\/\\/www\\.viator\\.com\\/tours\\/Orlando\\/[^"'\\s]+?\\/d663-([A-Z0-9_]+)/gi)) {
  ...
  for (const a of document.querySelectorAll('a[href*="d663-"]')) {
    const href = a.href || a.getAttribute('href') || '';
    const m = href.match(/d663-([A-Z0-9_]+)/i);
    if (m) {
