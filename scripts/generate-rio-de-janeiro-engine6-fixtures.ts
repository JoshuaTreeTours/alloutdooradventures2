/**
 * Generates Rio de Janeiro Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-rio-de-janeiro-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { RIO_DE_JANEIRO_VIATOR_PUBLIC_RATINGS } from "../src/engine6/rioDeJaneiroViatorPublicRatings";
...
const DESTINATION_CITY = "Rio de Janeiro";
const DESTINATION_STATE = "Brazil";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    "Ideal for visitors basing in Rio de Janeiro who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Rio de Janeiro sightseeing experience, with a guide handling hotel pickup, cable-car connections, and Corcovado or Sugar Loaf pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "Christ the Redeemer", stopType: "stop" as const },
      {
        title:
          first && first !== "Christ the Redeemer" ? "Sugar Loaf Mountain" : "Copacabana Beach",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    RIO_DE_JANEIRO_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Rio de Janeiro`,
            "Professional Rio de Janeiro guide",
          ].slice(0, 5)
        : [
            `${live.title} from Rio de Janeiro`,
            "Christ the Redeemer, Sugar Loaf, and Copacabana views",
            "Professional guide with Rio de Janeiro local expertise",
            "Small-group or private format as listed",
            "Guided Rio de Janeiro sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Rio de Janeiro meeting location listed when booking.",
    endDescription:
      "Return to your Rio de Janeiro meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    RIO_DE_JANEIRO_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Rio de Janeiro?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Rio de Janeiro Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    viatorDestinationSlug: "Rio-de-Janeiro",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Rio de Janeiro",
  });
};
