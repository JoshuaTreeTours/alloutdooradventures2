/**
 * Generates and applies Chicago Engine6 wiring from chicago-live-product-data.json
 * Run: npx tsx scripts/generate-chicago-engine6-wiring.ts
 */
...
const RATINGS_PATH = "src/engine6/chicagoViatorPublicRatings.ts";
const CATALOG_PATH = "scripts/chicago-product-catalog.json";
const LIVE_DATA_PATH = "scripts/chicago-live-product-data.json";

const ROUTE_PREFIX = "/destinations/illinois/chicago/tours/";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_CHICAGO_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type ChicagoViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
if (!routesContent.includes("ENGINE6_CHICAGO_TOUR_PATH_PREFIX")) {
  ...
  console.log(`Applied ${products.length} Chicago route constants and entries.`);
} else {
  console.log("Chicago routes already present in routes.ts.");
}
...
if (!fixturesContent.includes('productCode: "5580ARC"')) {
  ...
  console.log(
    `Applied ${products.length} Chicago validation fixture imports and entries.`
  );
} else {
  console.log("Chicago validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Chicago products.`);
