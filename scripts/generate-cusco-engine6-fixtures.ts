/**
 * Generates Cusco Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-cusco-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { CUSCO_VIATOR_PUBLIC_RATINGS } from "../src/engine6/cuscoViatorPublicRatings";
...
const DESTINATION_CITY = "Cusco";
const DESTINATION_STATE = "Peru";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  if (!destinationSignal.test(lead.slice(0, 150))) {
    ...
    if (lead.length > 150 || !destinationSignal.test(lead.slice(0, 150))) {
      lead =
        "Guided Cusco touring with local experts past Plaza de Armas, Coricancha, Sacsayhuaman, and the Sacred Valley.";
    }
  }
  const detail =
    sentences[1] ??
    "A local guide covers landmark stops such as Coricancha, Sacsayhuaman, Pisac, or Ollantaytambo depending on the itinerary.";
  const format =
    sentences[2] ??
    "Local commentary and route logistics are handled so visitors can focus on Cusco landmarks, Sacred Valley ruins, and highland trail stops.";
  const audience =
    "Ideal for visitors basing in Cusco who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (!/\bCusco\b/i.test(text)) {
    text = `${text} Departures are coordinated in Cusco, Peru.`;
  }
  ...
  const routeAnchor =
    landmarkNames.length > 0
      ...
      : "Plaza de Armas, Coricancha, and Sacred Valley landmarks around Cusco";
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Cusco sightseeing experience, with a guide handling hotel pickup, boat connections, and mountain or island pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
    text = `${text} Meeting points are confirmed at booking in Cusco, and the itinerary keeps visitors close to Plaza de Armas, Sacred Valley ruins, and highland trail stops that shape this Cusco tour.`;
  }
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "Plaza de Armas", stopType: "stop" as const },
      {
        title:
          first && first !== "Plaza de Armas" ? "Coricancha" : "Sacsayhuaman",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    CUSCO_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Cusco`,
            "Professional Cusco guide",
          ].slice(0, 5)
        : [
            `${live.title} from Cusco`,
            "Plaza de Armas, Sacred Valley, and highland trail views",
            "Professional guide with Cusco local expertise",
            "Small-group or private format as listed",
            "Guided Cusco sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Cusco meeting location listed when booking.",
    endDescription:
      "Return to your Cusco meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    CUSCO_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Cusco?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Cusco Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "peru",
    citySlug: "cusco",
    viatorDestinationSlug: "Cusco",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Cusco",
  });
};
