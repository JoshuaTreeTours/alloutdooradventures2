/**
 * Generates Lima Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-lima-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { LIMA_VIATOR_PUBLIC_RATINGS } from "../src/engine6/limaViatorPublicRatings";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const detail =
    sentences[1] ??
    "A local guide covers landmark stops such as the Pacific coast, the Plaza Mayor, the Arch of Lima, or Barranco depending on the itinerary.";
  const format =
    sentences[2] ??
  ...
  const audience =
    "Ideal for visitors basing in Lima who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Lima sightseeing experience, with a guide handling hotel pickup, boat connections, and mountain or island pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "the Pacific coast", stopType: "stop" as const },
      {
        title:
          first && first !== "the Pacific coast" ? "Plaza Mayor" : "Huaca Pucllana",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    LIMA_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Lima`,
            "Professional Lima guide",
          ].slice(0, 5)
        : [
            `${live.title} from Lima`,
            "the Pacific coast, Marcahuasi, and Plaza Mayor views",
            "Professional guide with Lima local expertise",
            "Small-group or private format as listed",
            "Guided Lima sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Lima meeting location listed when booking.",
    endDescription:
      "Return to your Lima meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    LIMA_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Lima?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Lima Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "peru",
    citySlug: "lima",
    viatorDestinationSlug: "Lima",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Lima",
  });
};
