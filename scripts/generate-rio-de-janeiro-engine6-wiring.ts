/**
 * Generates and applies Rio de Janeiro Engine6 wiring.
 * Run: npx tsx scripts/generate-rio-de-janeiro-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/rioDeJaneiroViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/rio-de-janeiro-product-catalog.json";
const LIVE_DATA_PATH =
  "scripts/rio-de-janeiro-live-product-data.json";
const EDITORIAL_OVERRIDES_PATH =
  "scripts/rio-de-janeiro-editorial-overrides.json";
const SELECTION_PATH =
  "scripts/rio-de-janeiro-product-selection.json";
const EDITORIAL_OVERRIDES = JSON.parse(
  readFileSync(EDITORIAL_OVERRIDES_PATH, "utf8")
) as Record<string, string>;
const NARRATIVES_PATH =
  "src/engine6/rioDeJaneiroApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/rio-de-janeiro-approved-narratives.json";

const ROUTE_PREFIX =
  "/destinations/brazil/rio-de-janeiro/tours/";
const ROUTES_INSERT_BEFORE =
  "export const ENGINE6_LIMA_20336P1_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_RIO_DE_JANEIRO_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const landmarkNames = live.itineraryStops
    ...
    .filter(
      name =>
        name &&
        !/^(Rio de Janeiro|Brazil|Rio)$/i.test(name) &&
  ...
  const destinationSignal = /\bRio de Janeiro\b/i;
  let lead =
    sentences[0] ??
    `Experience ${live.title.toLowerCase()} with a Rio de Janeiro guide.`;
  if (!destinationSignal.test(lead)) {
    lead = `In Rio de Janeiro, ${lead}`;
  }
  if (!destinationSignal.test(lead.slice(0, 150))) {
    lead = `Experience ${live.title} in Rio de Janeiro.`;
    if (lead.length > 150 || !destinationSignal.test(lead.slice(0, 150))) {
      lead =
        "Guided Rio de Janeiro touring with local experts past Christ the Redeemer, Sugar Loaf, Copacabana, and Ipanema.";
    }
  }
  const detail =
    sentences[1] ??
    "A local guide covers landmark stops such as Christ the Redeemer, Sugar Loaf, Copacabana Beach, or Escadaria Selarón depending on the itinerary.";
      ...
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ...
        : experienceType.includes("bike")
          ...
          : experienceType.includes("food") || experienceType.includes("cultural")
            ...
            : "Local commentary and route logistics are handled so visitors can focus on Rio de Janeiro landmarks, Corcovado, Sugar Loaf, and Tijuca Forest stops.");
  const audience =
    experienceType.includes("wine") || experienceType.includes("tequila")
      ? "Ideal for visitors basing in Rio de Janeiro who want a guided tasting day without coordinating rural driving or tasting-room appointments on their own."
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ? "Ideal for visitors basing in Rio de Janeiro who want a guided off-road day without renting ATVs independently."
        : experienceType.includes("bike")
          ? "Ideal for visitors basing in Rio de Janeiro who want a guided bike outing covering the coast without renting independently."
          : "Ideal for visitors basing in Rio de Janeiro who want a guided bay or mountain experience without coordinating tickets or landmark timing on their own.";
  ...
  let text = [lead, detail, landmarkSentence, format, audience]
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (!/\bRio de Janeiro\b/i.test(text)) {
    text = `${text} Departures are coordinated in Rio de Janeiro, Brazil.`;
  }
  ...
  const routeAnchor =
    landmarkNames.length > 0
      ...
      : "Christ the Redeemer, Sugar Loaf, Copacabana, and Ipanema";
  if (wordCount(text) < 120) {
    const logistics =
      experienceType.includes("wine") || experienceType.includes("tequila")
        ? "with a guide handling tasting timing, ranch pacing, and return transfers to Rio de Janeiro"
        : experienceType.includes("jeep") || experienceType.includes("offroad")
          ? "with a guide handling ATV briefing, jungle routing, and return to Rio de Janeiro"
          : experienceType.includes("bike")
    ...
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Rio de Janeiro experience, ${logistics} so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
    text = `${text} Meeting points are confirmed at booking in Rio de Janeiro, and the itinerary keeps visitors close to Corcovado, Sugar Loaf, Copacabana, and Tijuca Forest stops that shape this Rio de Janeiro tour.`;
  }
  ...
  text = text
    ...
    .replace(/\bLimited time in Rio de Janeiro\b/g, "A short stay in Rio de Janeiro")
    .replace(/\blimited time\b/gi, "a short stay")
    .replace(/^In Rio de Janeiro, ([A-Z])/g, (_match, letter: string) =>
      `In Rio de Janeiro, ${letter.toLowerCase()}`
    );
  return text;
};
...
const ratingsTs = `export type RioDeJaneiroViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const RIO_DE_JANEIRO_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type RioDeJaneiroTargetedNarrativeDescriptionProductCode =
  (typeof RIO_DE_JANEIRO_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const RIO_DE_JANEIRO_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  RioDeJaneiroTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getRioDeJaneiroTargetedNarrativeDescription = (
  productCode: string
) =>
  RIO_DE_JANEIRO_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as RioDeJaneiroTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (
  !routesContent.includes(
    "ENGINE6_RIO_DE_JANEIRO_TOUR_PATH_PREFIX"
  )
) {
  if (!routesContent.includes(ROUTES_INSERT_BEFORE)) {
    throw new Error("Missing Lima route constant anchor in routes.ts");
  }
  ...
  console.log(
    `Applied ${products.length} Rio de Janeiro route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes(`ENGINE6_RIO_DE_JANEIRO_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`)) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Lima route map anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Rio de Janeiro route map entries.`
  );
} else {
  console.log(
    "Rio de Janeiro route map entries already present."
  );
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  if (!fixturesContent.includes(FIXTURE_IMPORTS_INSERT_BEFORE)) {
    throw new Error(
      "Missing Lima fixture import anchor in validationFixtures.ts"
    );
  }
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error(
      "Missing Lima fixture entry anchor in validationFixtures.ts"
    );
  }
  ...
  console.log(
    `Applied ${products.length} Rio de Janeiro validation fixture imports and entries.`
  );
} else {
  console.log(
    "Rio de Janeiro validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Rio de Janeiro products.`
);
