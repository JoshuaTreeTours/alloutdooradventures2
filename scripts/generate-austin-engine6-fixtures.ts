/**
 * Generates Austin Engine6 fixtures from austin-live-product-data.json
 * Run: npx tsx scripts/generate-austin-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { AUSTIN_VIATOR_PUBLIC_RATINGS } from "../src/engine6/austinViatorPublicRatings";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    stops = [
      { title: "Austin Meeting Point", stopType: "stop" as const },
      { title: "Scenic Austin Viewpoint", stopType: "stop" as const },
    ];
  }
...
const buildTourFromLive = (live: LiveProduct): AustinTourFixture => {
  const priceFrom = parsePrice(live.priceFrom);
  const rating =
    live.rating ??
    AUSTIN_VIATOR_PUBLIC_RATINGS[live.productCode]?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
    AUSTIN_VIATOR_PUBLIC_RATINGS[live.productCode]?.reviewCount ??
    0;
  ...
  const highlights =
    overviewHighlights.length >= 3
      ? [
          ...overviewHighlights,
          `${live.title} departing from Austin`,
          "Professional Austin guide",
        ].slice(0, 5)
      : [
          `${live.title} from Austin`,
          "Texas State Capitol, Lady Bird Lake, South Congress, or Hill Country routes",
          "Professional guide with Austin local expertise",
          "Small-group or private format as listed",
          "Austin outdoor sightseeing, kayak, boat, or e-bike experience",
        ];
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Austin meeting location listed when booking, typically near the confirmed Austin address shown at booking.",
    endDescription:
      "Return to your Austin meeting point after the final stop on the itinerary.",
...
const AUSTIN_TOURS: AustinTourFixture[] = liveProducts.map(buildTourFromLive);

const buildFixture = (tour: AustinTourFixture) => {
  const viatorRatings = AUSTIN_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question: "Where does the tour depart from in Austin?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    for (const tour of AUSTIN_TOURS) {
    ...
    console.log(`Bootstrapped ${AUSTIN_TOURS.length} Austin Engine6 fixtures.`);
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    destinationLabel: "Austin",
    destinationCitySlug: "austin",
    stateSlug: "texas",
    citySlug: "austin",
    viatorDestinationSlug: "Austin",
    targetPremiumShare: 0.5,
    tours: AUSTIN_TOURS,
    buildFixture,
    destinationLogLabel: "Austin",
  });
};
