...
const codes = [...PUERTO_VALLARTA_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/mexico/puerto-vallarta/")) {
    issues.push("bad path");
  }
  if (!/Puerto Vallarta/i.test(governed)) {
    issues.push("no Puerto Vallarta in governed");
  }
  if (!/Puerto Vallarta/i.test(card)) {
    issues.push("no Puerto Vallarta in card excerpt");
  }
...
writeFileSync(
  "scripts/puerto-vallarta-render-surface-report.json",
