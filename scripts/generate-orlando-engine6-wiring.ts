/**
 * Generates and applies Orlando Engine6 wiring from orlando-live-product-data.json
 * Run: npx tsx scripts/generate-orlando-engine6-wiring.ts
 */
...
const RATINGS_PATH = "src/engine6/orlandoViatorPublicRatings.ts";
const CATALOG_PATH = "scripts/orlando-product-catalog.json";
const LIVE_DATA_PATH = "scripts/orlando-live-product-data.json";

const ROUTE_PREFIX = "/destinations/florida/orlando/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_KEY_WEST_TOUR_PATH_PREFIX";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_ORLANDO_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsTs = `export type OrlandoViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
if (!routesContent.includes("ENGINE6_ORLANDO_TOUR_PATH_PREFIX")) {
  routesContent = routesContent.replace(
    ROUTES_INSERT_BEFORE,
    `${routeBlock}${ROUTES_INSERT_BEFORE}`
  );
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Orlando route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_ORLANDO_3170P78_ROUTE,")) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Key West route anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Orlando route map entries.`);
} else {
  console.log("Orlando route map entries already present.");
}
...
if (!fixturesContent.includes('productCode: "3170P78"')) {
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error("Missing Key West fixture anchor in validationFixtures.ts");
  }
  ...
  console.log(
    `Applied ${products.length} Orlando validation fixture imports and entries.`
  );
} else {
  console.log("Orlando validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Orlando products.`);
