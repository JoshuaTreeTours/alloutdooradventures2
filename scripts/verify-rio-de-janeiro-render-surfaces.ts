...
const codes = [...RIO_DE_JANEIRO_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/brazil/rio-de-janeiro/")) {
    issues.push("bad path");
  }
  if (!/Rio de Janeiro/i.test(governed)) {
    issues.push("no Rio de Janeiro in governed");
  }
  if (!/Rio de Janeiro/i.test(card)) {
    issues.push("no Rio de Janeiro in card excerpt");
  }
...
writeFileSync(
  "scripts/rio-de-janeiro-render-surface-report.json",
