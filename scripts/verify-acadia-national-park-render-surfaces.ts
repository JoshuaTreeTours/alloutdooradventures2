...
import { ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/acadiaNationalParkViatorPublicRatings";
import {
...
const codes = [...ACADIA_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/maine/acadia-national-park/")) {
    issues.push("bad path");
  }
  if (!/Acadia/i.test(governed)) {
    issues.push("no Acadia in governed");
  }
  if (!/Acadia/i.test(card)) {
    issues.push("no Acadia in card excerpt");
  }
...
writeFileSync(
  "scripts/acadia-national-park-render-surface-report.json",
