...
const codes = [...LONDON_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/united-kingdom/london/")) {
    issues.push("bad path");
  }
  if (!/London/i.test(governed)) {
    issues.push("no London in governed");
  }
  if (!/London/i.test(card)) {
    issues.push("no London in card excerpt");
  }
...
writeFileSync(
  "scripts/london-render-surface-report.json",
