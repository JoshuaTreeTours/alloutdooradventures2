...
const codes = [...EDINBURGH_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!/Edinburgh/i.test(governed)) {
    issues.push("no Edinburgh in governed");
  }
  if (!/Edinburgh/i.test(card)) {
    issues.push("no Edinburgh in card excerpt");
  }
...
writeFileSync(
  "scripts/edinburgh-render-surface-report.json",
