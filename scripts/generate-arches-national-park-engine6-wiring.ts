/**
 * Generates and applies Arches National Park Engine6 wiring.
 * Run: npx tsx scripts/generate-arches-national-park-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/archesNationalParkViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/arches-national-park-product-catalog.json";
const LIVE_DATA_PATH =
  "scripts/arches-national-park-live-product-data.json";
const SELECTION_PATH =
  "scripts/arches-national-park-product-selection.json";
const NARRATIVES_PATH =
  "src/engine6/archesNationalParkApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/arches-national-park-approved-narratives.json";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_ARCHES_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const landmarkNames = live.itineraryStops
    ...
    .filter(
      name =>
        name &&
        !/^(Arches National Park|Utah)$/i.test(name) &&
  ...
  const destinationSignal = /\bArches(?: National Park)?\b/i;
  let lead =
    sentences[0] ??
    `Experience ${live.title.toLowerCase()} with an Arches National Park outfitter.`;
  if (!destinationSignal.test(lead)) {
    lead = `In Arches National Park, ${lead}`;
  }
  if (!destinationSignal.test(lead.slice(0, 150))) {
    lead = `Experience ${live.title} in Arches National Park, Utah.`;
    if (lead.length > 150 || !destinationSignal.test(lead.slice(0, 150))) {
      lead =
        "Guided Arches National Park touring with local experts through natural stone arches, fins, and red-rock viewpoints.";
    }
  }
  const detail =
    sentences[1] ??
    "Your guide covers landmark stops such as Delicate Arch, the Windows Section, Devils Garden, or nearby backcountry fins depending on the itinerary.";
  const format =
    sentences[2] ??
    "Local commentary and route logistics are handled so you can focus on arches, viewpoints, and photography or adventure highlights.";
  const audience =
    "Ideal for visitors basing in Moab who want a guided Arches National Park experience without coordinating timed entry, trailheads, or backcountry logistics on their own.";
  ...
  let text = [lead, detail, landmarkSentence, format, audience]
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (!/\bArches(?: National Park)?\b/i.test(text)) {
    text = `${text} Departures are coordinated near Arches National Park, Utah.`;
  }
  if (text.length < 500) {
    const routeAnchor =
      landmarkNames.length > 0
        ? landmarkNames.slice(0, 4).join(", ")
        : "Arches National Park red-rock corridors";
...
const ratingsTs = `export type ArchesNationalParkViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const ARCHES_NATIONAL_PARK_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;
...
if (
  !routesContent.includes(
    "ENGINE6_ARCHES_NATIONAL_PARK_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Arches National Park route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_ARCHES_108923P4_ROUTE,")) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Arches National Park route map entries.`
  );
} else {
  console.log(
    "Arches National Park route map entries already present."
  );
}
...
if (!fixturesContent.includes('productCode: "108923P4"')) {
  ...
  console.log(
    `Applied ${products.length} Arches National Park validation fixture imports and entries.`
  );
} else {
  console.log(
    "Arches National Park validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Arches National Park products.`
);
