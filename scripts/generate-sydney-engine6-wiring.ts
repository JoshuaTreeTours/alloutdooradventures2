/**
 * Generates and applies Sydney Engine6 wiring.
 * Run: npx tsx scripts/generate-sydney-engine6-wiring.ts
 */
...
const CATALOG_PATH = "scripts/sydney-product-catalog.json";
const LIVE_DATA_PATH = "scripts/sydney-live-product-data.json";
const SELECTION_PATH = "scripts/sydney-product-selection.json";
const APPROVED_NARRATIVES_JSON = "scripts/sydney-approved-narratives.json";
const EDITORIAL_OVERRIDES_PATH = "scripts/sydney-editorial-overrides.json";

const ROUTE_PREFIX = "/destinations/australia/sydney/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_OSAKA_92136P44_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_SYDNEY_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
if (!routesContent.includes("ENGINE6_SYDNEY_TOUR_PATH_PREFIX")) {
  if (!routesContent.includes(ROUTES_INSERT_BEFORE)) {
    throw new Error("Missing Osaka route constant anchor in routes.ts");
  }
  ...
  console.log(`Applied ${products.length} Sydney route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (
  !routesContent.includes(
    `ENGINE6_SYDNEY_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`
  )
) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Osaka route map anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Sydney route map entries.`);
} else {
  console.log("Sydney route map entries already present.");
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  if (!fixturesContent.includes(FIXTURE_IMPORTS_INSERT_BEFORE)) {
    throw new Error(
      "Missing Osaka fixture import anchor in validationFixtures.ts"
    );
  }
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error(
      "Missing Osaka fixture entry anchor in validationFixtures.ts"
    );
  }
  ...
  console.log(
    `Applied ${products.length} Sydney validation fixture imports and entries.`
  );
} else {
  console.log("Sydney validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Sydney products.`);
