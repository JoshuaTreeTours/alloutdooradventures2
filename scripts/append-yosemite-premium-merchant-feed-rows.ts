...
const newRows = NEW_YOSEMITE_PREMIUM_PRODUCT_CODES.map(productCode => {
...
console.log(
  `Appended ${newRows.length} Yosemite premium merchant feed rows (${existingLines.length - 1} -> ${existingLines.length - 1 + newRows.length}).`
);
