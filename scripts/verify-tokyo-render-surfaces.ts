...
const codes = [...TOKYO_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/japan/tokyo/")) {
    issues.push("bad path");
  }
  if (!/Tokyo/i.test(governed)) {
    issues.push("no Tokyo in governed");
  }
  if (!/Tokyo/i.test(card)) {
    issues.push("no Tokyo in card excerpt");
  }
...
writeFileSync(
  "scripts/tokyo-render-surface-report.json",
