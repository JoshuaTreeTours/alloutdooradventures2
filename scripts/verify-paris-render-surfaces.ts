...
const codes = [...PARIS_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/france/paris/")) {
    issues.push("bad path");
  }
  if (!/Paris/i.test(governed)) {
    issues.push("no Paris in governed");
  }
  if (!/Paris/i.test(card)) {
    issues.push("no Paris in card excerpt");
  }
...
writeFileSync(
  "scripts/paris-render-surface-report.json",
