...
for (const file of changedFiles) {
  if (
    ...
    try {
      catalogDiffs[file.path] = execSync(`git diff origin/main -- ${file.path}`, {
        encoding: "utf8",
      });
    } catch {
...
writeFileSync(
  "reports/buenos-aires-engine6-stage2-governance-audit.json",
  `${JSON.stringify(report, null, 2)}\n`
);
writeFileSync(
  "reports/buenos-aires-engine6-stage2-governance-audit.md",
