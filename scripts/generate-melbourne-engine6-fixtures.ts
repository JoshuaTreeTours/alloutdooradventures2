/**
 * Generates Melbourne Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-melbourne-engine6-fixtures.ts --bootstrap
 */
...
const DESTINATION_CITY = "Melbourne";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    throw new Error(
      `Melbourne product ${live.productCode} needs at least two verified itinerary stops`
    );
  }
...
const buildTourFromLive = (live: LiveProduct): MelbourneTourFixture => {
  const usdPrice = MELBOURNE_VIATOR_PUBLIC_USD_FROM_PRICES[live.productCode];
  const priceFrom = usdPrice ?? parsePrice(live.priceFrom);
  if (!usdPrice || priceFrom !== usdPrice) {
    throw new Error(
      `Melbourne product ${live.productCode} is missing a verified USD From price`
    );
  }
  const rating =
    live.rating ??
    MELBOURNE_VIATOR_PUBLIC_RATINGS[live.productCode]?.rating ??
    null;
  const reviewCount =
    live.reviewCount ??
    MELBOURNE_VIATOR_PUBLIC_RATINGS[live.productCode]?.reviewCount ??
    0;
  if (rating == null) {
    throw new Error(
      `Melbourne product ${live.productCode} is missing a verified rating`
    );
  }
  ...
  if (!description) {
    throw new Error(
      `Melbourne product ${live.productCode} is missing an approved narrative`
    );
  }
  ...
  return {
    ...
    startDescription:
      ...
      "Meet the guide at the confirmed Melbourne meeting location listed when booking.",
    endDescription:
      "Return to the Melbourne meeting point after the final stop on the itinerary.",
...
const MELBOURNE_TOURS: MelbourneTourFixture[] =
  liveProducts.map(buildTourFromLive);

const buildFixture = (tour: MelbourneTourFixture) => {
  const viatorRatings = MELBOURNE_VIATOR_PUBLIC_RATINGS[tour.productCode];
  const rating = viatorRatings?.rating ?? tour.rating;
  const reviewCount = viatorRatings?.reviewCount ?? tour.reviewCount;
  const usdPrice = MELBOURNE_VIATOR_PUBLIC_USD_FROM_PRICES[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question: "Where does the tour depart from for Melbourne?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    for (const tour of MELBOURNE_TOURS) {
    ...
    console.log(
      `Bootstrapped ${MELBOURNE_TOURS.length} Melbourne Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "australia",
    citySlug: "melbourne",
    viatorDestinationSlug: "Melbourne",
    targetPremiumShare: 0.3,
    tours: MELBOURNE_TOURS,
    buildFixture,
    destinationLogLabel: "Melbourne",
  });
};
