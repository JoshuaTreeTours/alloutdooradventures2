/**
 * Generates Bali Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-bali-engine6-fixtures.ts --bootstrap
 */
...
} from "../src/engine6/baliViatorPublicRatings";
...
const DESTINATION_CITY = "Bali";
const DESTINATION_STATE = "Indonesia";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    throw new Error(
      `Bali product ${live.productCode} needs at least two verified itinerary stops`
    );
  }
...
const buildTourFromLive = (live: LiveProduct): BaliTourFixture => {
  const usdPrice = BALI_VIATOR_PUBLIC_USD_FROM_PRICES[live.productCode];
  const priceFrom = usdPrice ?? parsePrice(live.priceFrom);
  if (!usdPrice || priceFrom !== usdPrice) {
    throw new Error(
      `Bali product ${live.productCode} is missing a verified USD From price`
    );
  }
  const rating =
    live.rating ??
    BALI_VIATOR_PUBLIC_RATINGS[live.productCode]?.rating ??
    null;
  const reviewCount =
    live.reviewCount ??
    BALI_VIATOR_PUBLIC_RATINGS[live.productCode]?.reviewCount ??
    0;
  if (rating == null) {
    throw new Error(
      `Bali product ${live.productCode} is missing a verified rating`
    );
  }
  ...
  if (!description) {
    throw new Error(
      `Bali product ${live.productCode} is missing an approved narrative`
    );
  }
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Bali meeting location listed when booking.",
    endDescription:
      "Return to your Bali meeting point after the final stop on the itinerary.",
...
const BALI_TOURS: BaliTourFixture[] = liveProducts.map(buildTourFromLive);

const buildFixture = (tour: BaliTourFixture) => {
  const viatorRatings = BALI_VIATOR_PUBLIC_RATINGS[tour.productCode];
  const rating = viatorRatings?.rating ?? tour.rating;
  const reviewCount = viatorRatings?.reviewCount ?? tour.reviewCount;
  const usdPrice = BALI_VIATOR_PUBLIC_USD_FROM_PRICES[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question: "Where does the tour depart from for Bali?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(`Bootstrapped ${BALI_TOURS.length} Bali Engine6 fixtures.`);
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "indonesia",
    citySlug: "bali",
    viatorDestinationSlug: "Bali",
    targetPremiumShare: 0.4,
    tours: BALI_TOURS,
    buildFixture,
    destinationLogLabel: "Bali",
  });
};
