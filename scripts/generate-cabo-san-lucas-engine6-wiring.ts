/**
 * Generates and applies Cabo San Lucas Engine6 wiring.
 * Run: npx tsx scripts/generate-cabo-san-lucas-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/caboSanLucasViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/cabo-san-lucas-product-catalog.json";
const LIVE_DATA_PATH =
  "scripts/cabo-san-lucas-live-product-data.json";
const EDITORIAL_OVERRIDES_PATH =
  "scripts/cabo-san-lucas-editorial-overrides.json";
const SELECTION_PATH =
  "scripts/cabo-san-lucas-product-selection.json";
const EDITORIAL_OVERRIDES = JSON.parse(
  readFileSync(EDITORIAL_OVERRIDES_PATH, "utf8")
) as Record<string, string>;
const NARRATIVES_PATH =
  "src/engine6/caboSanLucasApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/cabo-san-lucas-approved-narratives.json";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_CABO_SAN_LUCAS_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const landmarkNames = live.itineraryStops
    ...
    .filter(
      name =>
        name &&
        !/^(Cabo San Lucas|Mexico|Mexico)$/i.test(name) &&
  ...
  const destinationSignal = /\bCabo San Lucas\b/i;
  let lead =
    sentences[0] ??
    `Experience ${live.title.toLowerCase()} with a Cabo San Lucas guide.`;
  if (!destinationSignal.test(lead)) {
    lead = `In Cabo San Lucas, ${lead}`;
  }
  if (!destinationSignal.test(lead.slice(0, 150))) {
    lead = `Experience ${live.title} in Cabo San Lucas.`;
    if (lead.length > 150 || !destinationSignal.test(lead.slice(0, 150))) {
      lead =
        "Guided Cabo San Lucas touring with local experts past the Sea of Cortez, the El Arco, the Arch of Cabo San Lucas, and Land's End.";
    }
  }
  const detail =
    sentences[1] ??
    "A local guide covers landmark stops such as the Sea of Cortez, the El Arco, the Arch of Cabo San Lucas, or Land's End depending on the itinerary.";
      ...
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ...
        : experienceType.includes("bike")
          ...
          : experienceType.includes("food") || experienceType.includes("cultural")
            ...
            : "Local commentary and route logistics are handled so visitors can focus on Cabo San Lucas landmarks, the Sea of Cortez, and Sierra de la Laguna trail or island stops.");
  const audience =
    experienceType.includes("wine") || experienceType.includes("tequila")
      ? "Ideal for visitors basing in Cabo San Lucas who want a guided tasting day without coordinating rural driving or tasting-room appointments on their own."
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ? "Ideal for visitors basing in Cabo San Lucas who want a guided off-road day without renting ATVs independently."
        : experienceType.includes("bike")
          ? "Ideal for visitors basing in Cabo San Lucas who want a guided bike outing covering the coast without renting independently."
          : "Ideal for visitors basing in Cabo San Lucas who want a guided bay or mountain experience without coordinating tickets or landmark timing on their own.";
  ...
  let text = [lead, detail, landmarkSentence, format, audience]
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (!/\bCabo San Lucas\b/i.test(text)) {
    text = `${text} Departures are coordinated in Cabo San Lucas, Mexico.`;
  }
  ...
  const routeAnchor =
    landmarkNames.length > 0
      ...
      : "the Sea of Cortez landmarks around the El Arco and Cabo San Lucas Marina";
  if (wordCount(text) < 120) {
    const logistics =
      experienceType.includes("wine") || experienceType.includes("tequila")
        ? "with a guide handling tasting timing, ranch pacing, and return transfers to Cabo San Lucas"
        : experienceType.includes("jeep") || experienceType.includes("offroad")
          ? "with a guide handling ATV briefing, jungle routing, and return to Cabo San Lucas"
          : experienceType.includes("bike")
    ...
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Cabo San Lucas experience, ${logistics} so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
    text = `${text} Meeting points are confirmed at booking in Cabo San Lucas, and the itinerary keeps visitors close to the Sea of Cortez, Sierra de la Laguna trails, and Arch or Land's End stops that shape this Cabo San Lucas tour.`;
  }
  ...
  text = text
    ...
    .replace(/\bLimited time in Cabo San Lucas\b/g, "A short stay in Cabo San Lucas")
    .replace(/\blimited time\b/gi, "a short stay")
    .replace(/^In Cabo San Lucas, ([A-Z])/g, (_match, letter: string) =>
      `In Cabo San Lucas, ${letter.toLowerCase()}`
    );
  return text;
};
...
const ratingsTs = `export type CaboSanLucasViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const CABO_SAN_LUCAS_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type CaboSanLucasTargetedNarrativeDescriptionProductCode =
  (typeof CABO_SAN_LUCAS_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const CABO_SAN_LUCAS_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  CaboSanLucasTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getCaboSanLucasTargetedNarrativeDescription = (
  productCode: string
) =>
  CABO_SAN_LUCAS_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as CaboSanLucasTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (
  !routesContent.includes(
    "ENGINE6_CABO_SAN_LUCAS_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Cabo San Lucas route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_CABO_SAN_LUCAS_3714LUXSUNSET_ROUTE,")) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Cabo San Lucas route map entries.`
  );
} else {
  console.log(
    "Cabo San Lucas route map entries already present."
  );
}
...
if (!fixturesContent.includes('productCode: "3714LUXSUNSET"')) {
  ...
  console.log(
    `Applied ${products.length} Cabo San Lucas validation fixture imports and entries.`
  );
} else {
  console.log(
    "Cabo San Lucas validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Cabo San Lucas products.`
);
