/**
 * One-off discovery: live Viator public page data for Orlando catalog.
 * Run: npx tsx scripts/discover-orlando-viator-catalog.ts
 */
...
const CATALOG_URL = "https://www.viator.com/Orlando/d663-ttd";

const extractCatalogProducts = (html: string) => {
  const products = new Map<string, string>();

  for (const match of html.matchAll(
    /https:\/\/www\.viator\.com\/tours\/Orlando\/[^"'\s]+?\/d663-([A-Z0-9_]+)/gi
  )) {
    products.set(match[1].toUpperCase(), match[0]);
  }

  for (const match of html.matchAll(/\/tours\/Orlando\/[^"'\s]+?\/d663-([A-Z0-9_]+)/gi)) {
...
const REJECT_TITLE_PATTERN =
  /audio|self[- ]?guided|gps|app[- ]?based|smartphone|download|ticket only|entry ticket|admission only|universal studios ticket|disney ticket|seaworld ticket|legoland ticket/i;

const WEAK_COMMERCIAL_PATTERN =
  /transfer only|airport shuttle|one-way transfer|roundtrip transfer|parking pass|rental car/i;

const PREFERRED_PATTERN =
  /private|kennedy space|space center|nasa|airboat|everglades|helicopter|balloon|biplane|food|cultural|vip|premium|luxury|small group|guided|day trip|wildlife|gator|swamp|boat|cruise|kayak|fishing|sunset|sunrise|hot air|seaplane|flight|sightseeing|orlando/i;
...
const main = async () => {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const entry = {
        ...
        httpStatus: page.httpStatus,
        finalUrl: page.finalUrl,
        preferred: PREFERRED_PATTERN.test(
      ...
      const titleRejected =
        ...
        (REJECT_TITLE_PATTERN.test(extracted.title) ||
          WEAK_COMMERCIAL_PATTERN.test(extracted.title));
      const priceRejected =
      ...
      if (
        ...
        console.log(
          `SKIP ${candidate.productCode}: available=${availability.available} price=${extracted.priceFrom} title=${!!extracted.title}`
        );
      }
    } catch (error) {
      rejected.push({
  ...
  results.sort((a, b) => {
    ...
    const reviewsA = (a.reviewCount as number) ?? 0;
    const reviewsB = (b.reviewCount as number) ?? 0;
    return reviewsB - reviewsA;
  });

  writeFileSync(
    "scripts/orlando-viator-discovery-results.json",
    `${JSON.stringify({ catalogCount: candidates.length, available: results, rejected }, null, 2)}\n`
  );
  writeFileSync(
    "scripts/orlando-catalog-products.json",
    `${JSON.stringify(
      results.map(r => ({
