...
const codes = [...BUENOS_AIRES_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/argentina/buenos-aires/")) {
    issues.push("bad path");
  }
  if (!/Buenos Aires/i.test(governed)) {
    issues.push("no Buenos Aires in governed");
  }
  if (!/Buenos Aires/i.test(card)) {
    issues.push("no Buenos Aires in card excerpt");
  }
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/argentina/buenos-aires/"))
...
writeFileSync(
  "scripts/buenos-aires-render-surface-report.json",
