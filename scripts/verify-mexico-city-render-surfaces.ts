...
const codes = [...MEXICO_CITY_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/mexico/mexico-city/")) {
    issues.push("bad path");
  }
  if (!/Mexico City/i.test(governed)) {
    issues.push("no Mexico City in governed");
  }
  if (!/Mexico City/i.test(card)) {
    issues.push("no Mexico City in card excerpt");
  }
...
writeFileSync(
  "scripts/mexico-city-render-surface-report.json",
