/**
 * Append-only Bangkok commercial snapshot rows using the existing snapshot builder.
 * Does not rewrite preexisting snapshot rows.
 * Run: npx tsx scripts/append-bangkok-commercial-snapshot.ts
 */
...
const appended = BANGKOK_VIATOR_PUBLIC_PRODUCT_CODES.map(productCode => {
  const line = feedLines.find(entry => entry.split(",")[0] === productCode);
  if (!line) {
    throw new Error(`Missing merchant-feed row for Bangkok ${productCode}`);
  }
  const parsed = parseMerchantRow(line);
  if (!parsed) {
    throw new Error(`Unparseable merchant-feed row for Bangkok ${productCode}`);
  }
  if (!/ USD$/i.test(parsed.price)) {
    throw new Error(
      `Bangkok ${productCode} merchant-feed price is not USD: ${parsed.price}`
    );
  }
  return parsed;
...
if (appended.length === 0) {
  console.log("Commercial snapshot already contains Bangkok rows.");
  process.exit(0);
}
...
console.log(
  `Preserved ${snapshot.rows.length} existing commercial snapshot rows; appended ${appended.length} Bangkok rows.`
);
