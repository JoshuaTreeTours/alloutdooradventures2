...
import { BOULDER_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/boulderViatorPublicRatings";
import {
...
const codes = [...BOULDER_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/colorado/boulder/")) {
    issues.push("bad path");
  }
  if (!/Boulder/i.test(governed)) issues.push("no Boulder in governed");
  if (!/Boulder/i.test(card)) issues.push("no Boulder in card excerpt");
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/colorado/boulder/"))
...
writeFileSync(
  "/tmp/boulder-render-verify.json",
