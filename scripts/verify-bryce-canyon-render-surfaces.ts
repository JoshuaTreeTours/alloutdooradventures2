...
const codes = [...BRYCE_CANYON_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/utah/bryce-canyon-national-park/")) {
    issues.push("bad path");
  }
  if (!/Bryce Canyon/i.test(governed)) {
    issues.push("no Bryce Canyon in governed");
  }
  if (!/Bryce Canyon/i.test(card)) {
    issues.push("no Bryce Canyon in card excerpt");
  }
...
writeFileSync(
  "scripts/bryce-canyon-national-park-render-surface-report.json",
