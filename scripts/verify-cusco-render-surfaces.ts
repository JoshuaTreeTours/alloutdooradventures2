...
const codes = [...CUSCO_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/peru/cusco/")) {
    issues.push("bad path");
  }
  if (!/Cusco/i.test(governed)) {
    issues.push("no Cusco in governed");
  }
  if (!/Cusco/i.test(card)) {
    issues.push("no Cusco in card excerpt");
  }
...
writeFileSync(
  "scripts/cusco-render-surface-report.json",
