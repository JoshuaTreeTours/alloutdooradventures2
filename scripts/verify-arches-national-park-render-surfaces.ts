...
import { ARCHES_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/archesNationalParkViatorPublicRatings";
import {
...
const codes = [...ARCHES_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/utah/arches-national-park/")) {
    issues.push("bad path");
  }
  if (!/Arches/i.test(governed)) {
    issues.push("no Arches in governed");
  }
  if (!/Arches/i.test(card)) {
    issues.push("no Arches in card excerpt");
  }
...
writeFileSync(
  "scripts/arches-national-park-render-surface-report.json",
