...
import { KYOTO_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/kyotoViatorPublicRatings";
import {
...
const codes = [...KYOTO_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/japan/kyoto/")) {
    issues.push("bad path");
  }
  if (!/Kyoto/i.test(governed)) {
    issues.push("no Kyoto in governed");
  }
  if (!/Kyoto/i.test(card)) {
    issues.push("no Kyoto in card excerpt");
  }
...
writeFileSync(
  "scripts/kyoto-render-surface-report.json",
