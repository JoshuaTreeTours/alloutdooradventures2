...
const stringifyForTs = (value: unknown, depth = 0): string => {
  ...
  if (typeof value === "string") {
    return `"${escapeForTsString(value)}"`;
  }
...
const parseCsvRow = (row: Record<string, string>) => {
  ...
  const operator =
    sanitizeCsvText(row.operator) ||
    sanitizeCsvText(row.company_name) ||
    sanitizeCsvText(row.company_shortname);
  const shortDescription = sanitizeCsvText(row.short_description);
  const explicitCategory = sanitizeCsvText(row.category);
  const calendarLink = sanitizeCsvText(row.calendar_link) ?? "";
  const bookingUrl = (
    sanitizeCsvText(row.booking_url) ||
    calendarLink ||
  ...
  return {
    ...
    shortDescription,
    explicitCategory,
    tags: parseTags(sanitizeCsvText(row.tags) || ""),
    calendarLink,
    bookingUrl,
...
export const toursGenerated: Tour[] = ${stringifyForTs(tours)};
`;
