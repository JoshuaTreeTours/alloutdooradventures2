...
const codes = [...ROME_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/italy/rome/")) {
    issues.push("bad path");
  }
  if (!/Rome/i.test(governed)) {
    issues.push("no Rome in governed");
  }
  if (!/Rome/i.test(card)) {
    issues.push("no Rome in card excerpt");
  }
...
writeFileSync(
  "scripts/rome-render-surface-report.json",
