/**
 * Generates Bangkok Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-bangkok-engine6-fixtures.ts --bootstrap
 */
...
} from "../src/engine6/bangkokViatorPublicRatings";
...
const DESTINATION_CITY = "Bangkok";
const DESTINATION_STATE = "Thailand";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    throw new Error(
      `Bangkok product ${live.productCode} needs at least two verified itinerary stops`
    );
  }
...
const buildTourFromLive = (live: LiveProduct): BangkokTourFixture => {
  const usdPrice = BANGKOK_VIATOR_PUBLIC_USD_FROM_PRICES[live.productCode];
  const priceFrom = usdPrice ?? parsePrice(live.priceFrom);
  if (!usdPrice || priceFrom !== usdPrice) {
    throw new Error(
      `Bangkok product ${live.productCode} is missing a verified USD From price`
    );
  }
  const rating =
    live.rating ??
    BANGKOK_VIATOR_PUBLIC_RATINGS[live.productCode]?.rating ??
    null;
  const reviewCount =
    live.reviewCount ??
    BANGKOK_VIATOR_PUBLIC_RATINGS[live.productCode]?.reviewCount ??
    0;
  if (rating == null) {
    throw new Error(
      `Bangkok product ${live.productCode} is missing a verified rating`
    );
  }
  ...
  if (!description) {
    throw new Error(
      `Bangkok product ${live.productCode} is missing an approved narrative`
    );
  }
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Bangkok meeting location listed when booking.",
    endDescription:
      "Return to your Bangkok meeting point after the final stop on the itinerary.",
...
const BANGKOK_TOURS: BangkokTourFixture[] = liveProducts.map(buildTourFromLive);

const buildFixture = (tour: BangkokTourFixture) => {
  const viatorRatings = BANGKOK_VIATOR_PUBLIC_RATINGS[tour.productCode];
  const rating = viatorRatings?.rating ?? tour.rating;
  const reviewCount = viatorRatings?.reviewCount ?? tour.reviewCount;
  const usdPrice = BANGKOK_VIATOR_PUBLIC_USD_FROM_PRICES[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question: "Where does the tour depart from for Bangkok?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    for (const tour of BANGKOK_TOURS) {
    ...
    console.log(
      `Bootstrapped ${BANGKOK_TOURS.length} Bangkok Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "thailand",
    citySlug: "bangkok",
    viatorDestinationSlug: "Bangkok",
    targetPremiumShare: 0.4,
    tours: BANGKOK_TOURS,
    buildFixture,
    destinationLogLabel: "Bangkok",
  });
};
