/**
 * Append-only Singapore commercial snapshot rows using the existing snapshot builder.
 * Does not rewrite preexisting snapshot rows.
 * Run: npx tsx scripts/append-singapore-commercial-snapshot.ts
 */
import { readFileSync, writeFileSync } from "node:fs";

import { SINGAPORE_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/singaporeViatorPublicRatings";
import {
...
const appended = SINGAPORE_VIATOR_PUBLIC_PRODUCT_CODES.map(productCode => {
  const line = feedLines.find(entry => entry.split(",")[0] === productCode);
  if (!line) {
    throw new Error(`Missing merchant-feed row for Singapore ${productCode}`);
  }
  const parsed = parseMerchantRow(line);
  if (!parsed) {
    throw new Error(`Unparseable merchant-feed row for Singapore ${productCode}`);
  }
  if (!/ USD$/i.test(parsed.price)) {
    throw new Error(
      `Singapore ${productCode} merchant-feed price is not USD: ${parsed.price}`
    );
  }
  return parsed;
...
if (appended.length === 0) {
  console.log("Commercial snapshot already contains Singapore rows.");
  process.exit(0);
}
...
console.log(
  `Preserved ${snapshot.rows.length} existing commercial snapshot rows; appended ${appended.length} Singapore rows.`
);
