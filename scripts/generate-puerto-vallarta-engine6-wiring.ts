/**
 * Generates and applies Puerto Vallarta Engine6 wiring.
 * Run: npx tsx scripts/generate-puerto-vallarta-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/puertoVallartaViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/puerto-vallarta-product-catalog.json";
...
const NARRATIVES_PATH =
  "src/engine6/puertoVallartaApprovedNarrativeDescriptions.ts";
const APPROVED_NARRATIVES_JSON =
  "scripts/puerto-vallarta-approved-narratives.json";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_PUERTO_VALLARTA_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    experienceType.includes("wine") || experienceType.includes("tequila")
      ? "Ideal for visitors basing in Puerto Vallarta who want a guided tasting day without coordinating rural driving or tasting-room appointments on their own."
      : experienceType.includes("jeep") || experienceType.includes("offroad")
        ? "Ideal for visitors basing in Puerto Vallarta who want a guided off-road day without renting ATVs independently."
        : experienceType.includes("bike")
          ? "Ideal for visitors basing in Puerto Vallarta who want a guided bike outing covering the coast without renting independently."
          : "Ideal for visitors basing in Puerto Vallarta who want a guided bay or mountain experience without coordinating tickets or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    const logistics =
      experienceType.includes("wine") || experienceType.includes("tequila")
        ? "with a guide handling tasting timing, ranch pacing, and return transfers to Puerto Vallarta"
        : experienceType.includes("jeep") || experienceType.includes("offroad")
          ? "with a guide handling ATV briefing, jungle routing, and return to Puerto Vallarta"
          : experienceType.includes("bike")
            ? "with a cycling guide handling bikes, coastal pacing, and Malecón routing"
            : "with a guide handling ticket windows, coach or boat connections, and neighborhood pacing";
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Puerto Vallarta experience, ${logistics} so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const ratingsTs = `export type PuertoVallartaViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
const narrativesTs = `export const PUERTO_VALLARTA_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES = [
${narrativeCodes}
] as const;

export type PuertoVallartaTargetedNarrativeDescriptionProductCode =
  (typeof PUERTO_VALLARTA_TARGETED_NARRATIVE_DESCRIPTION_PRODUCT_CODES)[number];

export const PUERTO_VALLARTA_TARGETED_NARRATIVE_DESCRIPTIONS: Record<
  PuertoVallartaTargetedNarrativeDescriptionProductCode,
  string
> = {
${narrativeEntries}
};

export const getPuertoVallartaTargetedNarrativeDescription = (
  productCode: string
) =>
  PUERTO_VALLARTA_TARGETED_NARRATIVE_DESCRIPTIONS[
    productCode as PuertoVallartaTargetedNarrativeDescriptionProductCode
  ];
`;
...
if (
  !routesContent.includes(
    "ENGINE6_PUERTO_VALLARTA_TOUR_PATH_PREFIX"
  )
) {
  ...
  console.log(
    `Applied ${products.length} Puerto Vallarta route constants.`
  );
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (!routesContent.includes("ENGINE6_PUERTO_VALLARTA_2736P31_ROUTE,")) {
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(
    `Applied ${products.length} Puerto Vallarta route map entries.`
  );
} else {
  console.log(
    "Puerto Vallarta route map entries already present."
  );
}
...
if (!fixturesContent.includes('productCode: "2736P31"')) {
  ...
  console.log(
    `Applied ${products.length} Puerto Vallarta validation fixture imports and entries.`
  );
} else {
  console.log(
    "Puerto Vallarta validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Puerto Vallarta products.`
);
