/**
 * Generates Rome Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-rome-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { ROME_VIATOR_PUBLIC_RATINGS } from "../src/engine6/romeViatorPublicRatings";
...
const LIVE_DATA_PATH =
  "scripts/rome-live-product-data.json";
const DESTINATION_CITY = "Rome";
const DESTINATION_STATE = "Italy";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const destinationSignal = /\bRome\b/i;
  let lead =
    sentences[0] ??
    `Experience ${live.title.toLowerCase()} with a Rome guide.`;
  if (!destinationSignal.test(lead)) {
    lead = `In Rome, ${lead}`;
  }
  if (!destinationSignal.test(lead.slice(0, 150))) {
    lead = `Experience ${live.title} in Rome.`;
    if (lead.length > 150 || !destinationSignal.test(lead.slice(0, 150))) {
      lead =
        "Guided Rome touring with local experts past the Colosseum, the Roman Forum, the Pantheon, and the Trevi Fountain.";
    }
  }
  const detail =
    sentences[1] ??
    "Your guide covers landmark stops such as the Colosseum, Vatican Museums, Trastevere, or the Appian Way depending on the itinerary.";
  const format =
    sentences[2] ??
    "Local commentary and route logistics are handled so you can focus on Rome landmarks, piazzas, and neighborhood walking or cycling highlights.";
  const audience =
    "Ideal for visitors basing in Rome who want a guided city experience without coordinating Metro routes, attraction tickets, or landmark timing on their own.";
  ...
  let text = [lead, detail, landmarkSentence, format, audience]
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (!/\bRome\b/i.test(text)) {
    text = `${text} Departures are coordinated in Rome, Italy.`;
  }
  ...
  const routeAnchor =
    landmarkNames.length > 0
      ...
      : "central Rome landmarks around the historic center";
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Rome sightseeing experience, with a guide handling Metro connections, ticket windows, and neighborhood pacing so the day stays focused on the city rather than logistics.`;
  }
  if (wordCount(text) < 120) {
    text = `${text} Meeting points are confirmed at booking in Rome, and the itinerary keeps visitors close to piazza views, museum galleries, and historic streets that shape this Italian capital tour.`;
  }
  ...
  text = text
    ...
    .replace(/\bLimited time in Rome\b/g, "A short stay in Rome")
    .replace(/\blimited time\b/gi, "a short stay")
    .replace(/^In Rome, ([A-Z])/g, (_match, letter: string) =>
      `In Rome, ${letter.toLowerCase()}`
    );
  return text;
};
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    if (first === "Roman countryside vineyard") {
      stops = [
        { title: "Hotel pickup in Rome", stopType: "stop" as const },
        { title: "Roman countryside vineyard", stopType: "stop" as const },
      ];
    } else {
      stops = [
        { title: first || "Historic center of Rome", stopType: "stop" as const },
        {
          title: first && first !== "Colosseum" ? "Colosseum" : "Roman Forum",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    ROME_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Rome`,
            "Professional Rome guide",
          ].slice(0, 5)
        : [
            `${live.title} from Rome`,
            "Colosseum, Vatican Museums, and historic-center views",
            "Professional guide with Rome local expertise",
            "Small-group or private format as listed",
            "Guided Rome sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Rome meeting location listed when booking.",
    endDescription:
      "Return to your Rome meeting point after the final stop on the itinerary.",
...
const SELECTION_PATH =
  "scripts/rome-product-selection.json";
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    ROME_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Rome?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Rome Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "italy",
    citySlug: "rome",
    viatorDestinationSlug: "Rome",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Rome",
  });
};
