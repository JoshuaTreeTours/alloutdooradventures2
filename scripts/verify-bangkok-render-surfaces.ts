...
const codes = [...BANGKOK_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/thailand/bangkok/")) {
    issues.push("bad path");
  }
  if (!/Bangkok/i.test(governed)) {
    issues.push("no Bangkok in governed");
  }
  if (!/Bangkok/i.test(card)) {
    issues.push("no Bangkok in card excerpt");
  }
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/thailand/bangkok/"))
...
writeFileSync(
  "scripts/bangkok-render-surface-report.json",
