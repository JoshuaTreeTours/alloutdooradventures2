/**
 * One-off discovery: live Viator public page data for Key West catalog.
 * Run: npx tsx scripts/discover-key-west-viator-catalog.ts
 */
...
const extractCatalogProducts = (html: string) => {
  ...
  for (const match of html.matchAll(
    /https:\/\/www\.viator\.com\/tours\/Key-West\/[^"'\s]+?\/d661-([A-Z0-9_]+)/gi
  )) {
...
const PREFERRED_PATTERN =
  /private|charter|snorkel|sail|sunset|dry tortuga|food|history|ghost|kayak|eco|dolphin|wildlife|sandbar|boat|cruise|fishing|parasail|jet ski|reef|catamaran|yacht|seaplane|helicopter/i;
