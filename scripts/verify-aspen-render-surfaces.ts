...
import { ASPEN_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/aspenViatorPublicRatings";
import {
...
const codes = [...ASPEN_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/colorado/aspen/")) {
    issues.push("bad path");
  }
  if (!/Aspen/i.test(governed)) issues.push("no Aspen in governed");
  if (!/Aspen/i.test(card)) issues.push("no Aspen in card excerpt");
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/colorado/aspen/"))
