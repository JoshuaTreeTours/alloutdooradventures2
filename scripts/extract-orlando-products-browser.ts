/**
 * Playwright extraction of live Orlando Viator product pages.
 * Run: npx tsx scripts/extract-orlando-products-browser.ts
 */
...
const REJECT_TITLE_PATTERN =
  /audio|self[- ]?guided|gps|app[- ]?based|smartphone|download|admission ticket|entry ticket|theme park ticket|water park ticket|transfer only|airport transfer|port canaveral/i;

const WEAK_COMMERCIAL_PATTERN =
  /transfer only|airport shuttle|one-way transfer|roundtrip transfer|parking pass|rental car|port canaveral/i;

const PREFERRED_PATTERN =
  /private|kennedy space|space center|nasa|airboat|everglades|helicopter|balloon|biplane|food|cultural|vip|premium|luxury|small group|guided|day trip|wildlife|gator|swamp|boat|cruise|kayak|fishing|sunset|sunrise|hot air|seaplane|flight|sightseeing|manatee|clearwater|st\.? augustine|dinner show|luau|horseback|bioluminescent|safari|tree trek|murder mystery|pontoon|sailing/i;
...
const main = async () => {
  const catalog = (
    JSON.parse(
      readFileSync("scripts/orlando-catalog-products.json", "utf8")
    ) as CatalogEntry[]
