...
const codes = [...OSAKA_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/japan/osaka/")) {
    issues.push("bad path");
  }
  if (!/Osaka/i.test(governed)) {
    issues.push("no Osaka in governed");
  }
  if (!/Osaka/i.test(card)) {
    issues.push("no Osaka in card excerpt");
  }
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/japan/osaka/"))
...
writeFileSync(
  "scripts/osaka-render-surface-report.json",
