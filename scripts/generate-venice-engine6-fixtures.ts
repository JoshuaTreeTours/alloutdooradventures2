/**
 * Generates Venice Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-venice-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { VENICE_VIATOR_PUBLIC_RATINGS } from "../src/engine6/veniceViatorPublicRatings";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const format =
    sentences[2] ??
    "Local commentary and route logistics are handled so you can focus on Venice landmarks, piazzas, and neighborhood walking or cycling highlights.";
  const audience =
    "Ideal for visitors basing in Venice who want a guided city experience without coordinating vaporetto routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Venice sightseeing experience, with a guide handling vaporetto connections, ticket windows, and neighborhood pacing so the day stays focused on the city rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    if (first === "Prosecco Hills wineries") {
      stops = [
        { title: "Piazzale Roma", stopType: "stop" as const },
        { title: "Prosecco Hills wineries", stopType: "stop" as const },
      ];
    } else {
      stops = [
        { title: first || "Historic center of Venice", stopType: "stop" as const },
        {
          title: first && first !== "St. Mark's Basilica" ? "St. Mark's Basilica" : "Rialto Bridge",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    VENICE_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Venice`,
            "Professional Venice guide",
          ].slice(0, 5)
        : [
            `${live.title} from Venice`,
            "St. Mark's Basilica, Doge's Palace, and historic-center views",
            "Professional guide with Venice local expertise",
            "Small-group or private format as listed",
            "Guided Venice sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Venice meeting location listed when booking.",
    endDescription:
      "Return to your Venice meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    VENICE_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Venice?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Venice Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    destinationCitySlug: "venice",
    stateSlug: "italy",
    citySlug: "venice",
    viatorDestinationSlug: "Venice",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Venice",
  });
};
