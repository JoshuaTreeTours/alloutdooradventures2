...
import { CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/canyonlandsNationalParkViatorPublicRatings";
import {
...
const codes = [...CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/utah/canyonlands-national-park/")) {
    issues.push("bad path");
  }
  if (!/Canyonlands/i.test(governed)) {
    issues.push("no Canyonlands in governed");
  }
  if (!/Canyonlands/i.test(card)) {
    issues.push("no Canyonlands in card excerpt");
  }
...
writeFileSync(
  "scripts/canyonlands-national-park-render-surface-report.json",
