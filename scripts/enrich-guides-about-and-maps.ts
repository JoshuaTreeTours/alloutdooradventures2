...
import { buildCityAboutFactGroups } from "../src/utils/guides/buildCityAboutBlurb";
...
const run = async () => {
  ...
  for (const file of files) {
    ...
    const factGroups = buildCityAboutFactGroups({
      cityName: guide.city,
    ...
    lengthReport.push({
      ...
      state: guide.state,
      tier,
      wordCount: toWords(factGroups.map(group => group.text).join(" ")),
    });
