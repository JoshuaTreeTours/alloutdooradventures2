...
const codes = [...CAIRNS_VIATOR_PUBLIC_PRODUCT_CODES];
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/australia/cairns/")) {
    issues.push("bad path");
  }
  if (!/Cairns/i.test(governed)) {
    issues.push("no Cairns in governed");
  }
  if (!/Cairns/i.test(card)) {
    issues.push("no Cairns in card excerpt");
  }
  ...
  const expectedUsd = CAIRNS_VIATOR_PUBLIC_USD_FROM_PRICES[code];
  const expectedRating = CAIRNS_VIATOR_PUBLIC_RATINGS[code];
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/australia/cairns/"))
...
writeFileSync(
  "scripts/cairns-render-surface-report.json",
