/**
 * Generates Edinburgh Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-edinburgh-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { EDINBURGH_VIATOR_PUBLIC_RATINGS } from "../src/engine6/edinburghViatorPublicRatings";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const format =
    sentences[2] ??
    "Local commentary and route logistics are handled so visitors can focus on Edinburgh landmarks, castle views, and neighborhood walking or cycling highlights.";
  const audience =
    "Ideal for visitors basing in Edinburgh who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Edinburgh sightseeing experience, with a guide handling coach or walking connections, ticket windows, and Highland or Old Town pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "Edinburgh Castle", stopType: "stop" as const },
      {
        title:
          first && first !== "Edinburgh Castle" ? "Royal Mile" : "Arthur's Seat",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    EDINBURGH_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Edinburgh`,
            "Professional Edinburgh guide",
          ].slice(0, 5)
        : [
            `${live.title} from Edinburgh`,
            "Edinburgh Castle, the Royal Mile, and Old Town views",
            "Professional guide with Edinburgh local expertise",
            "Small-group or private format as listed",
            "Guided Edinburgh sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Edinburgh meeting location listed when booking.",
    endDescription:
      "Return to your Edinburgh meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    EDINBURGH_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Edinburgh?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Edinburgh Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    citySlug: "edinburgh",
    viatorDestinationSlug: "Edinburgh",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Edinburgh",
  });
};
