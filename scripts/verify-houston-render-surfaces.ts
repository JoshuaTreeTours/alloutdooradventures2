...
import { HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/houstonViatorPublicRatings";
import {
...
const codes = [...HOUSTON_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/texas/houston/")) {
    issues.push("bad path");
  }
  if (!/Houston/i.test(governed)) issues.push("no Houston in governed");
  if (!/Houston/i.test(card)) issues.push("no Houston in card excerpt");
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/texas/houston/"))
...
writeFileSync(
  "/tmp/houston-render-verify.json",
