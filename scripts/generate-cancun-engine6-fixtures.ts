/**
 * Generates Cancun Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-cancun-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { CANCUN_VIATOR_PUBLIC_RATINGS } from "../src/engine6/cancunViatorPublicRatings";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    "Ideal for visitors basing in Cancun who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Cancun sightseeing experience, with a guide handling hotel pickup, ruin tickets, and cenote or island pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "Chichen Itza", stopType: "stop" as const },
      {
        title:
          first && first !== "Chichen Itza" ? "Isla Mujeres" : "Puerto Morelos",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    CANCUN_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Cancun`,
            "Professional Cancun guide",
          ].slice(0, 5)
        : [
            `${live.title} from Cancun`,
            "Chichen Itza, cenotes, and Caribbean coastline views",
            "Professional guide with Cancun local expertise",
            "Small-group or private format as listed",
            "Guided Cancun sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Cancun meeting location listed when booking.",
    endDescription:
      "Return to your Cancun meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    CANCUN_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Cancun?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Cancun Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    stateSlug: "mexico",
    citySlug: "cancun",
    viatorDestinationSlug: "Cancun",
    targetPremiumShare: 0.5,
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Cancun",
  });
};
