/**
 * Generates Cabo San Lucas Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-cabo-san-lucas-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { CABO_SAN_LUCAS_VIATOR_PUBLIC_RATINGS } from "../src/engine6/caboSanLucasViatorPublicRatings";
...
const DESTINATION_CITY = "Cabo San Lucas";
...
const buildEditorialDescription = (live: LiveProduct) => {
  ...
  const audience =
    "Ideal for visitors basing in Cabo San Lucas who want a guided city experience without coordinating bus and walking routes, attraction tickets, or landmark timing on their own.";
  ...
  if (wordCount(text) < 120) {
    text = `${text} Routes stay oriented to ${routeAnchor} that define this Cabo San Lucas sightseeing experience, with a guide handling hotel pickup, boat connections, and mountain or island pacing so the day stays focused on the outing rather than logistics.`;
  }
  if (wordCount(text) < 120) {
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    ...
    stops = [
      { title: first || "the Sea of Cortez", stopType: "stop" as const },
      {
        title:
          first && first !== "the Sea of Cortez" ? "El Arco" : "the Arch of Cabo San Lucas",
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    CABO_SAN_LUCAS_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const rawHighlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Cabo San Lucas`,
            "Professional Cabo San Lucas guide",
          ].slice(0, 5)
        : [
            `${live.title} from Cabo San Lucas`,
            "the Sea of Cortez, Sierra de la Laguna, and El Arco views",
            "Professional guide with Cabo San Lucas local expertise",
            "Small-group or private format as listed",
            "Guided Cabo San Lucas sightseeing with local commentary",
          ];
  const highlights = [
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Cabo San Lucas meeting location listed when booking.",
    endDescription:
      "Return to your Cabo San Lucas meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    CABO_SAN_LUCAS_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Cabo San Lucas?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Cabo San Lucas Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Cabo San Lucas",
  });
};
