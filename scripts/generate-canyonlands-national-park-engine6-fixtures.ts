/**
 * Generates Canyonlands National Park Engine6 fixtures from live product data.
 * Run: npx tsx scripts/generate-canyonlands-national-park-engine6-fixtures.ts --bootstrap
 */
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

import { CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS } from "../src/engine6/canyonlandsNationalParkViatorPublicRatings";
...
const DESTINATION_CITY = "Canyonlands National Park";
...
const resolveItineraryItems = (live: LiveProduct): ItineraryItem[] => {
  ...
  if (stops.length < 2) {
    stops = [
      { title: "Canyonlands National Park", stopType: "stop" as const },
      { title: "Island in the Sky", stopType: "stop" as const },
    ];
  }
...
const buildTourFromLive = (live: LiveProduct): BryceTourFixture => {
  ...
  const rating =
    live.rating ??
    CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS[live.productCode]
      ?.rating ??
    5.0;
  const reviewCount =
    live.reviewCount ??
  ...
  const highlights =
    live.highlights && live.highlights.length >= 3
      ...
      : overviewHighlights.length >= 3
        ? [
            ...overviewHighlights,
            `${live.title} near Canyonlands National Park`,
            "Professional Canyonlands National Park guide",
          ].slice(0, 5)
        : [
            `${live.title} from Canyonlands National Park`,
            "Island in the Sky overlooks and red-rock canyon views",
            "Professional guide with Canyonlands local expertise",
            "Small-group or private format as listed",
            "Canyonlands scenic, adventure, or photography experience",
          ];
  ...
  return {
    ...
    startDescription:
      ...
      "Meet your guide at the confirmed Canyonlands meeting location listed when booking, typically near Moab or the park entrance area shown at booking.",
    endDescription:
      "Return to your Moab or Canyonlands National Park meeting point after the final stop on the itinerary.",
...
const buildFixture = (tour: BryceTourFixture) => {
  const viatorRatings =
    CANYONLANDS_NATIONAL_PARK_VIATOR_PUBLIC_RATINGS[tour.productCode];
  ...
  return {
    product: {
      ...
      faqs: [
        ...
        {
          question:
            "Where does the tour depart from for Canyonlands National Park?",
...
const main = async () => {
  if (process.argv.includes("--bootstrap")) {
    ...
    console.log(
      `Bootstrapped ${BRYCE_TOURS.length} Canyonlands National Park Engine6 fixtures.`
    );
    return;
  }
  ...
  await runEngine6ParagonFixtureGeneration({
    ...
    tours: BRYCE_TOURS,
    buildFixture,
    destinationLogLabel: "Canyonlands National Park",
  });
};
