...
const codes = [...BALI_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/indonesia/bali/")) {
    issues.push("bad path");
  }
  if (!/Bali/i.test(governed)) {
    issues.push("no Bali in governed");
  }
  if (!/Bali/i.test(card)) {
    issues.push("no Bali in card excerpt");
  }
...
const listingCodes = engine6ResolvedTours
  .filter(tour => tour.canonicalPath.includes("/indonesia/bali/"))
...
writeFileSync(
  "scripts/bali-render-surface-report.json",
