/**
 * Generates and applies Rocky Mountain National Park Engine6 wiring.
 * Run: npx tsx scripts/generate-rocky-mountain-national-park-engine6-wiring.ts
 */
...
const RATINGS_PATH =
  "src/engine6/rockyMountainNationalParkViatorPublicRatings.ts";
const CATALOG_PATH =
  "scripts/rocky-mountain-national-park-product-catalog.json";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_RMNP_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
const ratingsEntries = products
  .map(p => {
    ...
    return `  "${p.productCode}": { rating: ${rating}, reviewCount: ${reviewCount} },`;
  })
  .join("\n");

const ratingsTs = `export type RockyMountainNationalParkViatorPublicRating = {
  rating: number;
  reviewCount: number;
};
...
if (!routesContent.includes("ENGINE6_ROCKY_MOUNTAIN_NATIONAL_PARK_TOUR_PATH_PREFIX")) {
  ...
  console.log(
    `Applied ${products.length} Rocky Mountain National Park route constants and entries.`
  );
} else {
  console.log(
    "Rocky Mountain National Park routes already present in routes.ts."
  );
}
...
if (!fixturesContent.includes('productCode: "366391P1"')) {
  ...
  console.log(
    `Applied ${products.length} Rocky Mountain National Park validation fixture imports and entries.`
  );
} else {
  console.log(
    "Rocky Mountain National Park validation fixtures already present."
  );
}

console.log(
  `Generated wiring for ${products.length} Rocky Mountain National Park products.`
);
