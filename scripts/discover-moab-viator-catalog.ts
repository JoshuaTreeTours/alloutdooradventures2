/**
 * One-off discovery: live Viator public page data for Moab catalog.
 * Run: npx tsx scripts/discover-moab-viator-catalog.ts
 */
...
const CATALOG_URL = "https://www.viator.com/Moab/d5600-ttd";

const extractCatalogProducts = (html: string) => {
  const products = new Map<string, string>();

  for (const match of html.matchAll(
    /https:\/\/www\.viator\.com\/tours\/Moab\/[^"'\s]+?\/d5600-([A-Z0-9_]+)/gi
  )) {
    products.set(match[1].toUpperCase(), match[0]);
  }

  for (const match of html.matchAll(/\/tours\/Moab\/[^"'\s]+?\/d5600-([A-Z0-9_]+)/gi)) {
