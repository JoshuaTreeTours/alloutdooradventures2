...
const codes = [...SINGAPORE_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/singapore/singapore/")) {
    issues.push("bad path");
  }
  if (!/Singapore/i.test(governed)) {
    issues.push("no Singapore in governed");
  }
  if (!/Singapore/i.test(card)) {
    issues.push("no Singapore in card excerpt");
  }
...
writeFileSync(
  "scripts/singapore-render-surface-report.json",
