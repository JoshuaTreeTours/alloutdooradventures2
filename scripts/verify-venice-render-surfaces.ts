...
import { VENICE_VIATOR_PUBLIC_PRODUCT_CODES } from "../src/engine6/veniceViatorPublicRatings";
import {
...
const codes = [...VENICE_VIATOR_PUBLIC_PRODUCT_CODES];
const bleed = [
  /Yellowstone/i,
  /Yosemite/i,
  /\bZion\b/i,
...
for (const code of codes) {
  ...
  if (!tour.canonicalPath.includes("/italy/venice/")) {
    issues.push("bad path");
  }
  if (!/Venice/i.test(governed)) {
    issues.push("no Venice in governed");
  }
  if (!/Venice/i.test(card)) {
    issues.push("no Venice in card excerpt");
  }
...
writeFileSync(
  "scripts/venice-render-surface-report.json",
