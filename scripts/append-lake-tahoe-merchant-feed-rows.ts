...
const newRows = LAKE_TAHOE_VIATOR_PUBLIC_PRODUCT_CODES.map(productCode => {
...
console.log(
  `Appended ${newRows.length} Lake Tahoe merchant feed rows (${existingLines.length - 1} -> ${existingLines.length - 1 + newRows.length}).`
);
