/**
 * Generates and applies Queenstown Engine6 wiring.
 * Run: npx tsx scripts/generate-queenstown-engine6-wiring.ts
 */
...
const CATALOG_PATH = "scripts/queenstown-product-catalog.json";
...
const APPROVED_NARRATIVES_JSON = "scripts/queenstown-approved-narratives.json";
const EDITORIAL_OVERRIDES_PATH = "scripts/queenstown-editorial-overrides.json";

const ROUTE_PREFIX = "/destinations/new-zealand/queenstown/tours/";
const ROUTES_INSERT_BEFORE = "export const ENGINE6_CAIRNS_3253P11_PRODUCT_CODE";
...
const toConstName = (productCode: string, suffix: string) =>
  `ENGINE6_QUEENSTOWN_${productCode.replace(/[^A-Z0-9]/gi, "_")}_${suffix}`;
...
if (!routesContent.includes("ENGINE6_QUEENSTOWN_TOUR_PATH_PREFIX")) {
  if (!routesContent.includes(ROUTES_INSERT_BEFORE)) {
    throw new Error("Missing Cairns route constant anchor in routes.ts");
  }
  ...
  console.log(`Applied ${products.length} Queenstown route constants.`);
}

routesContent = readFileSync(ROUTES_PATH, "utf8");
if (
  !routesContent.includes(
    `ENGINE6_QUEENSTOWN_${products[0].productCode.replace(/[^A-Z0-9]/gi, "_")}_ROUTE,`
  )
) {
  ...
  if (!normalizedRoutes.includes(normalizedAnchor)) {
    throw new Error("Missing Cairns route map anchor in routes.ts");
  }
  ...
  writeFileSync(ROUTES_PATH, routesContent, "utf8");
  console.log(`Applied ${products.length} Queenstown route map entries.`);
} else {
  console.log("Queenstown route map entries already present.");
}
...
if (!fixturesContent.includes(`productCode: "${products[0].productCode}"`)) {
  if (!fixturesContent.includes(FIXTURE_IMPORTS_INSERT_BEFORE)) {
    throw new Error(
      "Missing Cairns fixture import anchor in validationFixtures.ts"
    );
  }
  ...
  if (!normalizedFixtures.includes(normalizedFixtureAnchor)) {
    throw new Error(
      "Missing Cairns fixture entry anchor in validationFixtures.ts"
    );
  }
  ...
  console.log(
    `Applied ${products.length} Queenstown validation fixture imports and entries.`
  );
} else {
  console.log("Queenstown validation fixtures already present.");
}

console.log(`Generated wiring for ${products.length} Queenstown products.`);
